<template>
    <transition name="vue-avatar" mode="out-in">
        ...
        <div class="vue-avatar" :style="style" :key="src">

<!--            <span v-if="!image_exists">{{ initials }}</span>-->
<!--          <img :style="style" v-if="!image_exists" src="http://www.gravatar.com/avatar/227dda8fadad6eb6f1ea65b459818837?d=mm&s=24"/>-->
        </div>
    </transition>
</template>
        ...
        computed: {
            ...
            style() {
                ...
                const default_styles = {
                    display: 'inline-flex',
                    backgroundColor: 'transparent',//`hsl(${hue}, ${saturation}%, ${lightness}%)`,
                    ...
                    fontSize: '12px'//Math.floor(this.size / 2.5) + 'px',
                }

                if (this.src.includes('undefined')||this.src.includes('null')) {
                    Object.assign(default_styles, {
                        backgroundImage: `url(${require('@/assets/用户.png')})`,
