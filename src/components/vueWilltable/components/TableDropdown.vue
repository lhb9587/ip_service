<template>
  <div class="km-dropdown-wrapper" :class="{ active: parseInt(dropdown.index) >= 0 }" :style="{
      'top': `${$parent.$refs.wrapper.offsetTop + 30}px`,
      'left': left
    }" ref="dropdown" v-clickoutside="openDropdown">
    <div class="km-sort">
    ...
    <div class="km-filter">
      ...
      <ul class="content">
        <li>
          <el-checkbox-group v-model="checkList" @change="changeCheck" style="display: flex;flex-direction: column">
            <el-checkbox size="mini" v-for="(item, key) in dropdown.list" :label="key" :key="key" style="margin-right: 2px;">
              <p>
...
<!--          <el-checkbox size="mini" v-model="item.checked" style="margin-right: 2px;">-->
<!--          </el-checkbox>-->
<!--          <p>-->
<!--            <span :title="key">{{ key }}</span>-->
<!--            <i>（{{ item.count }}）</i>-->
  ...
  methods: {
    changeCheck(value){
      if(value && value.length){
        value.forEach(item => {
          Object.keys(this.dropdown.list).forEach(j => {
            if (value.includes(j)){
              this.dropdown.list[j].checked = true
            }else{
              this.dropdown.list[j].checked = false
            }
          })
