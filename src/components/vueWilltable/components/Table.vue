<template>
  <div ref="willtable" class="km-willtable">
    <div v-if="store.states.columns.length > 0" ref="wrapper" class="km-table-wrapper" :class="{ scrollX: tableWidth > wrapperWidth }" :style="{ maxWidth: `${tableWidth}px` }"
      v-clickoutside="clickoutside">
      <div class="km-table-header" ref="theader">
        <table-header ref="theaderContent" :showIcon="showIcon" :columnsWidth="columnsWidth" :fixedCount="fixedCount" :all-show="true" :store="store" />
      </div>
      <div ref="tbody" class="km-table-body" :style="{maxHeight: `${maxHeight}px`}">
        <table-body ref="tbodyContent" :columnsWidth="columnsWidth" :all-show="true" :cellStyle="cellStyle" :cellClassName="cellClassName" :rowHeight="rowHeight" :store="store">
          <!-- 编辑器 -->
          <editor ref="editor" :columnsWidth="columnsWidth" :fixedCount="fixedCount" :rowHeight="rowHeight" :autofillState="autofillState" :store="store" :plType="plType"/>
        </table-body>
      ...
      <div ref="fixedWrapper" class="km-table-fixed" :style="{width: `${fixedWidth}px`}">
        <div class="km-table-fixed-header" ref="fixedTheader">
          <table-header ref="fixedTheaderContent" :showIcon="showIcon" :columnsWidth="columnsWidth" :fixedCount="fixedCount" :store="store" />
        </div>
        <div ref="fixedTbody" class="km-table-fixed-body">
          <table-body ref="fixedTbodyContent"  :columnsWidth="columnsWidth" :cellStyle="cellStyle" :cellClassName="cellClassName" :rowHeight="rowHeight" :store="store" />
        </div>
      </div>
    </div>
    ...
    <dropdown ref="dropdown" :columnsWidth="columnsWidth" :fixedCount="fixedCount" :store="store"></dropdown>
...
export default {
  ...
  computed:{
    currentNum(){
      return this.store.states.showData.length
    }
  },
  props: {
    columns: {
   ...
   console.log(n,'当前距离')
    ...
    data: {
      handler(val) {
        ...
        this.handleResize();
        this.$refs.dropdown.resetFilter('force')
      },
      deep: true
    },
    columns: {
  ...
  methods: {
    ...
    initData() {
      ...
      states.rowSelection=this.rowSelection
    ...
    handleResize() {
      ...
      this.$nextTick(() => {
        ...
        if (allWidth) {
          this.tableWidth = states.columns
            .map(cell => cell.width)
            .reduce((a, b) => a + b, 0);
        } else {
          this.tableWidth = this.columnsWidth.reduce(
            (total, cur) => total + cur,
            0
          );
        ...
        this.$nextTick(() => {
          if (this.tableWidth > this.wrapperWidth) {
            this.$refs.fixedWrapper.style.height = `${this.$refs.wrapper
              .offsetHeight - states.scrollBarWidth}px`;
            this.$refs.fixedTbody.style.height = `${this.$refs.wrapper
              .offsetHeight -
            ...
            this.$refs.fixedTbody.style.height = `${this.$refs.wrapper
              .offsetHeight - this.theaderHeight}px`;
          }
        });
        // 当出现竖向滚动条时处理滚动条
        if (this.$refs.tbodyContent.$el.offsetHeight > this.maxHeight) {
          ...
          this.$nextTick(() => {
            const colgroup = this.$refs.theaderContent.$el.querySelector(
              "colgroup"
            );
            ...
            this.$refs.tbodyContent.$el.style.width = `${this.tableWidth -
              states.scrollBarWidth -
              1}px`;
          });
        }
   ...
   //数组中判断某一属性是否都相同
   isSame(list,key){
      const firstEditor = list[0][key];
        for (let i = 1; i < list.length; i++) {
          if (list[i][key] !== firstEditor) {
              return false;
          }
        }
      return true
    ...
    selectCell(e, x, y, type) {
     console.log('选择了')
      ...
      const { states } = this.store;
      console.log(states.showData,'this.showData');
      if (states.columns[x].autofill == false) {
        this.autofillState = false;
      } else {
        //特殊处理：批量处理当境外代理所不同时，银行账户下拉填充功能取消
        if (states.columns[x].key == "account") {
          const list = states.showData
          const isSameStauts = this.isSame(list,'abroadAgencyCustId')
          this.autofillState = isSameStauts;
        }else{
          this.autofillState = true;
        }
      }
   this.mselectdataArr.push({x:x,y:y,con:states.showData[y][states.columns[x].key]})
  if(this.mselectdataArr.length>2){
    this.mselectdataArr.shift();
  }
  if(e.shiftKey){

         if(this.mselectdataArr.length==2){
            if(this.mselectdataArr[0].x== this.mselectdataArr[1].x){
                if(this.mselectdataArr[0].y<this.mselectdataArr[1].y){

                  for (let index = this.mselectdataArr[0].y; index <= this.mselectdataArr[1].y; index++) {

                    this.store.mgetEditorContent(this.mselectdataArr[0].con,x,index);
                  }
                }else{
                   for (let index = this.mselectdataArr[1].y; index <= this.mselectdataArr[0].y; index++) {
                    this.store.mgetEditorContent(this.mselectdataArr[1].con,x,index);
                  }
                }
                  this.mselectdataArr=[];
            }else{
      ...
      setTimeout(() => {
        ...
        for (let i = 0; i <= arr.length - 1; i += 1) {
          for (let j = 0; j <= arr[i].length - 1; j += 1) {
            if (
              this.store.states.showData[
                i + this.store.states.selector.selectedYArr[0]
              ] &&
              this.store.states.columns[
                j + this.store.states.selector.selectedXArr[0]
              ]
            ) {
              if (
                !this.store.states.columns[
    ...
    setEditing(key) {
      const { states } = this.store;
      if (
        states.columns[states.editor.editorXIndex].disabled &&
        states.columns[states.editor.editorXIndex].type != "tmCaseList"
      ) {
        return;
      }
      states.editor.editType = states.columns[states.editor.editorXIndex].type
      ...
      if (states.editor.editType === "select") {
        ...
        //   states.editor.key = states.columns[states.editor.editorXIndex].key;
        // }
      }
      this.$nextTick(() => {
        this.$refs.editor.$refs[states.editor.editType]&&this.$refs.editor.$refs[states.editor.editType].focus();
      });
    },
    selectUp() {
