<template>
  <table cellspacing="0" cellpadding="0" style="position: relative;">
    ...
    <thead class="km-thead">
      <tr ref="tr">
        <th v-for="(th, index) in columns" :key="index" :style="{
              width: `${columnsWidth[index]}px`,
              'text-align': th.align
          }" :title="th.title" v-show="th.fixed || allShow">
          <el-checkbox v-if="th.type === 'selection'" size="mini" v-model="checkedAll" @change="selectAll">
          </el-checkbox>
          <p class="km-cell-content" :style="{width: `${columnsWidth[index] - 20}px`}" :class="{active: isActive(columnsStatusList[index])}" v-else>
            <span class="icon" :class="iconClass(th.type)" v-if="showIcon"></span>
            <span class="content">{{ th.title }}</span>
          </p>
          <div v-if="th.type != 'selection'" class="dropdown" :class="{active: dropdown.index === index}">
            <i v-if="th.action" @click.stop="openDropdown(index)"></i>
          </div>
          <div v-if="th.type != 'selection'" class="handler" @mousedown="handlerDown(index)">
          </div>
        </th>
      </tr>
    </thead>
