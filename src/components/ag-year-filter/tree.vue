  ...
  export default {
    ...
    created(){
      this.defaultTreeData=JSON.parse(JSON.stringify(this.data))
       this.treeData=this.c(this.getTreeData(this.data.map(item => item.abslimitDate)))
    ...
    methods:{
      filterNode(value, data) {
        ...
        let str= getTwoDimensionalArray(this.treeData, 'id', data.id, {children: 'child'}).map(i => i.split('-')[1]).join('-')
     ...
     c(arr) {
      ...
      check(checkedNodes,checkedKeys,halfCheckedNodes,halfCheckedKeys){
        let arr= checkedKeys.checkedNodes.map(item => getTwoDimensionalArray(this.treeData, 'id', item.id, {children: 'child'}))
        let keys= arr.filter(item => item.length ==3).map(item => item.map(i => i.split('-')[1]).join('-'))
