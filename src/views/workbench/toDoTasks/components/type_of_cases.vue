  ...
  <div>
        ...
        <div class="buttonWrap">
          ...
          <el-dropdown trigger="click"  @command="changeSonMenuState" placement="bottom-start">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item command="c" v-allow="112">上传报官清单回执</el-dropdown-item>
              <el-dropdown-item command="d" v-allow="115">官方通知审核</el-dropdown-item>
              <el-dropdown-item command="批量转交">批量转交</el-dropdown-item>
    ...
    >
      ...
      <el-table-column v-for="(item,index) in tableDataList" :key="index" :label="item.title" align="left" :prop="item.value" :width="item.width?item.width:'auto'"
        ...
        <template slot-scope="scope">
          <div :class="item.value=='agentNum'&&scope.row.pageId!==101?'itemTitle_style':''" :style="{cursor: scope.row.pageId!==101? 'pointer':''}" @click="scope.row.pageId!==101?$router.push('/workbench/case/seeCaseDetail/'+scope.row.caseId):''">
            ...
            <div v-if="item.value==='materialArray'&&item.title==='上传文件'">
              <p v-for="i in scope.row.materialArray">
                <a style="display: block;color: #409EFF;" target="_blank" :href="'ipdoc'+i.address">{{ i.name }}</a>
              </p>
      ...
      <el-table-column label="操作" align="left" fixed="right"  class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            ||
            scope.row.pageId==160
            ||
            scope.row.pageId==161
            ||
            scope.row.pageId==162
          "
          >处理</el-button>
<!--          <el-button-->
<!--            type="text"-->
...
<!--            @click="transfromtask(scope.row)">任务权限转交</el-button>-->
    ...
    <el-dialog
      ...
      v-dialogDrag
    >
      <detailSubmission :query="detailSubmissionData" @cancel="closeHandover"></detailSubmission>
    </el-dialog>
...
<!--    商标撰写-->
    <el-dialog
      ...
      v-dialogDrag
    >
      <trademark-writing-detail v-if="trademarkWritingView" :atData="atData" :type="type" :taskNo="taskNo"
                                :taskIdList="taskIdList" :caseIds="trademarkWritingCaseIdList"
...
import { queryAlltask, queryInstruction, submitOfficalAudit, auditOfficalDoc, queryDocs ,lowCreditAudit,taskFinishTask} from '@/api/caseList.js'
import HandOver  from "./HandOver";
  import TrademarkWritingDetail from "../../case/trademarkWriting/trademarkWritingDetail";
export default {
  components: {TrademarkWritingDetail, Pagination, DetailOfficialCommunication ,HandOver,detailSubmission},
  name: 'index',
  props:{
    refesh:{
      default:false
  ...
  data() {
    return {
      ...
      multipleTypeText: '确认',
      tableDefaultList:[{'title':'案件文号','value':'agentNum','width': 200 },{'title':'案件类型','value':'caseType'},{'title':'案件名称','value':'tmName'},{'title':'客户','value':'custName'},{'title':'申请人','value':'appCnName'}, {'title':'发起人','value':'fullname','width': 110 }, {'title':'案件阶段','value':'stageName','width': 120 },{'title':'案件最新进展','value':'proceeName' },{'title':'案件个数','value':'caseNumber',width:120 },{'title':'案件状态','value':'statusName' },{'title':'备注','value':'memo' }],
      tableSubmissonList:[{'title':'案件文号','value':'agentNum','width': 200 },{'title':'案件类型','value':'caseType'},{'title':'递交方式','value':'submitMode' },{'title':'上报事项','value':'reportMatter'},{'title':'是否缺材料','value':'isLoadPriorityFile' },{'title':'递交场所','value':'submitPlace'},
        {'title':'递交日期','value':'submitDate' },{'title':'文件类型','value':'materials'},{'title':'文件名称','value':'materials' },{'title':'上传文件','value':'materials'},{'title':'下达人','value':'submitUser'},{'title':'备注','value':'memo' }],
      tableOfficialList:[{'title':'案件文号','value':'agentNum','width': 200},{'title':'案件类型','value':'caseType'},{'title':'客户','value':'custName'},{'title':'申请人','value':'appCnName'},{'title':'官文类型','value':'typeName'},{'title':'文件名称','value':'docName'},
        { 'title': '发文日期', 'value': 'docDate' }, { 'title':'收文日期','value':'recvDate'},{'title':'邮寄方式','value': 'mailingTypeName'}, {'title': '上传文件', 'value': 'materialArray','width': 200 }, {'title': '提交人', 'value': 'creater','width': 110 } ,{'title':'备注','value':'memo' }],
      tableDataList: [ ],
      currentTypeText: '',
      dialogVisible:false,
      queryInfo: { caseIds: '' },
  ...
  methods: {
    ...
    jumpMassfiling(row) {
      if ([160, 161, 162].includes(row.pageId)) {
        if (row.taskNo) {
          this.taskIdList = [row.taskId]
      ...
      if (row.pageId == 100 || row.pageId == 150 || row.pageId == 151) {
        this.$router.push({ path: '/workbench/case/case_detail/' + row.caseId+'/'+row.caseType, query: { pageId: row.pageId,caseTypeId:row.caseTypeId,createStatus:row.createStatus }})
      }
      if (row.pageId == 101 || row.pageId == 152 || row.pageId == 153) {
        this.$router.push({ path: '/workbench/case/massfiling', query: { taskId: row.taskId, deliverId: row.deliverId ,isChangeCase:row.isChangeCase,createStatus:row.createStatus}})
      }
      if (row.pageId==102) {
        this.$router.push({ path: '/workbench/case/preFilingPush/2', query: { taskId: row.taskId }})
      }
      if (row.pageId==103&&row.proceeName=='网申递交失败') {
        this.detailSubmissionData={ caseIds: row.caseId, pageId: row.pageId, pageType:'4',insId:row.insId,taskIdList:row.taskId }
      ...
      if (row.pageId==103&&row.proceeName!=='网申递交失败') {
        this.detailSubmissionData={ caseIds: row.caseId, pageId: row.pageId, pageType:'1',insId:row.insId,taskIdList:row.taskId }
        this.detailSubmissionState=true
        // this.$router.push({ path: '/workbench/case/detailSubmission', query: { caseIds: row.caseId, pageId: row.pageId, pageType:'1',insId:row.insId,taskIdList:row.taskId }})
      }
      if (row.pageId==104) {
        this.detailSubmissionData={ caseIds: row.caseId, pageId: row.pageId ,pageType:'2' ,insId:row.insId}
        this.detailSubmissionState=true
      ...
      if (row.pageId==107) {
        this.$router.push({ path: '/workbench/case/preFilingPush/3', query: { taskId: row.taskId }})
      }
      if(row.pageId==105){
        this.queryInfo={
          caseIds: row.caseId,
      ...
      queryAlltask(Object.assign(data, this.query)).then(res => {
        ...
        if(this.currentTypeText==='官方通知'){
          ...
          queryDocs({ userId: this.$store.getters.userId ,insIdList:this.taskList.filter(item => item.pageId==106).map(item => item.insId)}).then(res=>{
            ...
            this.taskList.forEach(caseItem=>{
              res.data.forEach((item,index) => {
                if(item.tmFileId == caseItem.insId) {
                  for(var key in item){
                    this.$set(caseItem, key, item[key])
                  }
                }
