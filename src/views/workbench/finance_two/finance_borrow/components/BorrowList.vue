...
<!--          <img @click="exportList(2)" src="@/assets/moon.png"  width="16" alt="">-->
    ...
    <SelectOption :buss-id="bussId" v-if="selectionOptionState" :dialog-visible="selectionOptionState" @cancel="closeSelect" :idArray="getCurrentRowsData('loanId')" :defaultMultipleSelect="preferenceList.map(i=>i.title)" :exportType="exportType" :exportQueryModel="exportQueryModel"></SelectOption>
    <BorrowDetail ref="borrowDetail" @cancel="cancel"/>
  </div>
</template>
...
import {queryLoanList, deleteLoan, doLoanTask, selectColumn, queryPreference, delPreference, savePreference} from '@/api/caseList.js'
...
export default {
  ...
  methods: {
    ...
    cellRenderer(params) {
      ...
      if(['loanTotal', 'beiJingLoanMoney', 'roundTripMoney', 'trafficMoney', 'hotelMoney', 'hospitalityMoney', 'businessMoney', 'officeMoney', 'laborMoney', 'otherMoney'].includes(params.colDef.field)){
    ...
    // 改签
    rebook(row){
      this.$emit('events', {action: 'BORROW_REBOOK', data: {...row, rightKeySign: true}})
    },
    // 双击查看
    rowDoubleClicked(params){
      ...
      // 当前任务人 + 审核权限 + 流程状态  = 查看审核
      // 当前任务人 + 审核权限 + 可编辑 + 流程状态 = 编辑审核
      // 不符合上面操作 = 查看
      if(
        isTaskPerson && this.$store.getters.permissions.includes(310) && row.taskNo == 2
        ||
        isTaskPerson && this.$store.getters.permissions.includes(311) && row.taskNo == 3
        ||
        isTaskPerson && this.$store.getters.permissions.includes(312) && row.taskNo == 4
      ){
        if (row.flag){
          this.$refs.borrowDetail.borrowAudit(JSON.parse(JSON.stringify(row)),'edit');
        }else{
          this.$refs.borrowDetail.borrowAudit(row,'view');
        }
      }else{
        this.$refs.borrowDetail.borrowView(row);
      }
    },
    // 获取数量
    getSum(list,type){
    ...
    getRowContextmenuList(row){
      ...
      list = [
        { name: '复制', action: 'copy' },
        { name: '修改', action: 'rowClickEdit', permissions: 344 },
        { name: '改签', action: 'rebook' },
        { name: '删除', action: 'rowClickDelete', permissions: 345 }
      ]
    ...
    queryList(){
      ...
      queryLoanList(querData).then(res=>{
...
// 拖动中
    ...
    // 借款创建
    borrowCreate() {
      this.$refs.borrowDetail.borrowCreate();
    },
    // 借款右键编辑
    rowClickEdit(row) {
      if (row.flag){
    ...
    rowClickDelete(row) {
      if (row.flag){
        ...
        this.$message.error('该借款您无法删除！');
        return
      }
    },
    // 借款编辑
    borrowEdit(row) {
      this.$refs.borrowDetail.borrowEdit(JSON.parse(JSON.stringify(row)));
    },
    // 借款提交
    borrowSubmit(row) {
      this.$refs.borrowDetail.borrowSubmit(JSON.parse(JSON.stringify(row)));
    },
    // 借款删除
    borrowDelete() {
      ...
      var loanIdList = []
      if (list.length < 1) {
        this.$message.error('请先选择借款数据');
        return
      } else {
        list.forEach(item => {
          item.loanId && loanIdList.push(item.loanId)
        })
      }
      this.$confirm('此操作将永久删除' + list.length + '条借款, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteLoan({ loanIds: loanIdList.join(',') }).then(res => {
          if (res.success) {
            this.queryList()
            // this.cancel()
          }
    ...
    // 借款状态更改(做任务)
    doLoanTask(taskNo, result) {
      var list = this.getSelectedRows().filter(item=>item.taskNo!==taskNo)
      if (list.length){
        this.$message.error(`有${list.length}条借款流程状态不符合当前操作！`)
        return false
      }
  ...
  computed:{
    ...
    pinnedBottomRowData(){
      if (this.gridApi.rowModel) {
        const columnDisplay = ['loanTotal', 'beiJingLoanMoney', 'roundTripMoney', 'trafficMoney', 'hotelMoney', 'hospitalityMoney', 'businessMoney', 'officeMoney', 'laborMoney', 'otherMoney']
