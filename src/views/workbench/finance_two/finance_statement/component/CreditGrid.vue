<template>
  <div>
    <div class="grid-table" :style="{width:'100%',height: statementTopHeight}">
      <!--      <div class="dots" @click="handleCheckboxState">-->
      <!--        <span class="dot">.</span>-->
      <!--        <span class="dot">.</span>-->
      <!--        <span class="dot">.</span>-->
      <!--      </div>-->
      <div style="margin-top: 5px;height: 28px;">
<!--        <el-button size="mini" type="primary" @click="createCredit">新建</el-button>-->
<!--        <el-button size="mini" type="primary" @click="delCredits">删除</el-button>-->
      </div>
      ...
      <div style="height: 30px;line-height: 30px;margin-top: 5px;font-size: 13px;color: #606266">共 {{list.length}} 条</div>
<!--      <pagination-->
<!--        v-if="total"-->
<!--          :total="total"-->
...
<script>
  ...
  import {queryLevel, deleteteLevel} from '@/api/billApi'
  ...
  export default {
    ...
    computed: {
      gridHeight() {
        return Number(this.statementTopHeight.replace('px', '')) - 70 + 'px'
      }
    },
    props: {
      ctx:{},
    ...
    data() {
      ...
      return {
        ...
        preferenceList: [
          {'title': '信用等级', 'value': 'level'},
          {'title': '设定时间', 'value': 'createDate'},
          {'title': '设定人', 'value': 'userIdStr'},
        ],
        selectColumnList: [],
        checkCaseList: [],
        bussId: 8,
    ...
    created() {
      this.queryTitle()
      Bus.$on('queryCreditList', () => {
        this.queryList()
      })
    },
    beforeDestroy() {
      Bus.$off('queryCreditList')
    },
    methods: {
      brushRight() {
      ...
      createCredit(){
        dispatcher(this.ctx)('STATEMENT_SET_CREDIT', {})
        // this.ctx.
      },
      getSelectedRows() {
        if (this.gridApi) {
      ...
      rowDoubleClicked(params) {
        dispatcher(this.ctx)('STATEMENT_EDIT_CREDIT', {...params.data, eventType: '查看'})
      },
      ...
      editCredit(row){
        dispatcher(this.ctx)('STATEMENT_EDIT_CREDIT', {...row, eventType: '修改'})
      },
      delCredit(row){
        this.$confirm('此操作将永久删除该条目, 是否继续?', '提示', {
          ...
          deleteteLevel({custCreditId: row.custCreditId}).then(res => {
      ...
      getRowContextmenuList(row) {
        return [
          {name: '复制', action: 'copy'},
          // {name: '修改', action: 'editCredit'},
          // {name: '删除', action: 'delCredit'},
        ].filter(item => {
          if (item.flag) {
      ...
      queryList() {
        ...
        queryLevel({...data, custId: this.custId}).then(res => {
          this.list = res.data
