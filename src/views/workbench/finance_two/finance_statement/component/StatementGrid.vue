<template>
  <div>
    <div style="margin-top: 5px">
      <el-button size="mini" type="primary" @click="createCheckBill">新建</el-button>
      <el-button size="mini" type="primary" @click="delCheckBills">删除</el-button>
    </div>
...
<script>
  ...
  import {queryDunningList, deleteDunning} from '@/api/billApi'
  ...
  export default {
    ...
    created() {
      this.queryTitle()
      Bus.$on('queryCheckBillList', () => {
        this.queryList()
      })
    },
    beforeDestroy() {
      Bus.$off('queryCheckBillList')
    },
    methods: {
      brushRight() {
      ...
      delCheckBill(row) {
        this.$confirm('此操作将永久删除该条目, 是否继续?', '提示', {
          ...
          deleteDunning({dnIds: row.dnId}).then(res => {
      ...
      delCheckBills() {
        ...
        this.$confirm('此操作将永久删除该条目, 是否继续?', '提示', {
          ...
          deleteDunning({dnIds: this.getSelectedRows().map(item => item.dnId).join(',')}).then(res => {
            this.$message.success('删除成功!')
            this.queryList()
          })
        })
      ...
      rowDoubleClicked(params) {
        dispatcher(this.ctx)('STATEMENT_EDIT_STATEMENT', {...params.data, eventType: '查看'})
      },
      ...
      editCheckBill(row) {
        dispatcher(this.ctx)('STATEMENT_EDIT_STATEMENT', {...row, eventType: '修改'})
      },
      getRowContextmenuList(row) {
        return [
          {name: '复制', action: 'copy'},
          {name: '修改', action: 'editCheckBill'},
          {name: '删除', action: 'delCheckBill'},
        ].filter(item => {
          if (item.flag) {
      ...
      cellRenderer(params) {
        ...
        if (Object.prototype.toString.call(params.value).includes('Number')) {
          return `<span title="${params.value}">${formatAmount(params.value)}</span>`
        }
      ...
      queryList() {
        ...
        queryDunningList({...data, custId: this.custId}).then(res => {
          this.list = res.data
