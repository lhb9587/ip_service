  ...
  <div>
    <div class="grid-table" :style="{width:'100%',height: statementTopHeight}">
      ...
      <div style="margin-top: 5px">
        <el-button size="mini" type="primary" @click="createMemo">新建</el-button>
<!--        <el-button size="mini" type="primary" @click="delMemos">删除</el-button>-->
      </div>
    ...
    <el-dialog
      ...
      class="abow_dialog"
    >
      <memo-detail ref="memo_detail" :editData="editData" :custName="custName" :custId="custId" v-if="memoDialogVisible" :memoType="memoType" @closeDialog="closeDialog"/>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import {queryDMemo,deleteDMemo} from '@/api/billApi'
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        preferenceList: [
          {'title': '催款相关备注', 'value': 'description'},
          {'title': '备注', 'value': 'memo'},
          {'title': '客户', 'value': 'custIdStr'},
        ],
        selectColumnList: [],
        checkCaseList: [],
        bussId: 8,
    ...
    methods: {
      ...
      delMemo(row) {
        this.$confirm('此操作将永久删除该条目, 是否继续?', '提示', {
          ...
          deleteDMemo({custDmId: row.custDmId}).then(res=>{
      ...
      queryList() {
        ...
        queryDMemo({...data, custId: this.custId}).then(res => {
          this.list = res.data
