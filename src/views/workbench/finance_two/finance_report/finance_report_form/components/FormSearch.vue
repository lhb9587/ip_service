          ...
          <transition name="show" mode="out-in">
            <div class="searchWrap" id="searchWrap" v-show="drawerState">
              <div class="form-container">
                <el-form v-if="name == 'finance_report_form'" size="mini" label-width="120px" @keyup.enter.native="handleSearch(0)" style="margin-top: 2px">
                  <el-form-item label="报表类型:">
                    <el-select v-model="queryModuleData.action" filterable clearable @change="typeCheckout">
                      <el-option
                        v-for="item in ctx.tabList"
                        :key="item.action"
                        :label="item.label"
                        :value="item.value"
                      >
                  ...
                  <el-form-item label="所属单位:">
                    <el-select v-model="queryModuleData.deptId" filterable clearable>
                      <el-option
                  ...
                  <el-form-item label="填报日期:">
                    <DataPicker class="DataPicker"
                                style="width: 400px"
                                v-model="queryModuleData.fillinDateArray"
                                type="date" value-format="yyyy-MM-dd"></DataPicker>
                  </el-form-item>
                </el-form>
              </div>
...
export default {
  ...
  created() {
    this.queryModuleData = JSON.parse(localStorage.getItem('queryModuleData')) ? JSON.parse(localStorage.getItem('queryModuleData')).find(item => item.name == this.name) ? JSON.parse(localStorage.getItem('queryModuleData')).find(item => item.name == this.name).queryModuleData : JSON.parse(JSON.stringify(this.defaultModuleData)) : JSON.parse(JSON.stringify(this.defaultModuleData))
    if (this.queryModuleData.action) {
      // 判断报表，所属月所属组处理
      if (this.queryModuleData.action == 'salary') {
        this.isMonth = false
        this.isQuarter = true
        if (this.queryModuleData.belongToYear && this.queryModuleData.quarter) {
          this.yearQuarter = this.queryModuleData.belongToYear + '-' + this.queryModuleData.quarter
          this.belongToYear = this.queryModuleData.belongToYear
          this.quarter = this.queryModuleData.quarter
        }
      } else {
        this.isMonth = true
        this.isQuarter = false
        this.belongToDateArray = this.queryModuleData.belongToDateArray
      }
    }
    this.queryCompany()
  },
  methods: {
    ...
    typeCheckout(row) {
      const quarterData = this.queryModuleData
      if (row == 'salary') {
        this.isMonth = false
        this.isQuarter = true
        for (var key in quarterData) {
          if (['belongToDateArray'].includes(key)) {
        ...
        this.isQuarter = false
        for (var key in quarterData) {
          if (['belongToYear','quarter'].includes(key)) {
            delete quarterData[key]
          }
        }
      }
      let newList = {}
      if (this.isMonth) {
        newList = { belongToDateArray: this.belongToDateArray }
      } else {
        newList = { belongToYear: this.belongToYear, quarter: this.quarter }
      }
      this.queryModuleData = Object.assign(quarterData, newList)
    },
    openOrClose(){
    ...
    handleSearch(flag) {
      ...
      if (flag == 1 && !this.queryModuleData.hasOwnProperty('deptId')){
        this.queryModuleData.deptId = ''
      }
      if (this.queryModuleData.action == 'salary') {
        this.queryModuleData.belongToYear = this.belongToYear
        this.queryModuleData.quarter = this.quarter
      } else {
        this.queryModuleData.belongToDateArray = this.belongToDateArray
      }
      this.drawerState = false
