    ...
    <div v-if="activeName" style="display: flex;">
      ...
      <div class="right_wrap">
        ...
        <div>
          <keep-alive>
            <!--          数据列存在渲染-->
            <component v-if="checkList.length > 0" v-show="activeIndex == '概览' ? $store.getters.permissions.includes(398) : true" :is="activeType" :active-index="activeIndex" :check-list="checkList"></component>
          </keep-alive>
        </div>
      </div>
    </div>
...
import logs from './components/dataPanel/logs'
import hrm from './components/dataPanel/hrm'
export default {
  components: { Sidebar, kpi, customer, partner, logs, hrm },
  data() {
    return {
      activeName: '',
      activeType: '',
      ...
      defaultTabList: [
        {name:'业绩数据',type:'kpi'},
        {name:'客户数据',type:'customer',permissions :404},
        {name:'合伙人表现',type:'partner',permissions :362},
        {name:'访问日志',type:'logs',permissions :479},
        // {name:'人力资源',type:'hrm'}
      ],
      tabList: [],
      leftWrapWidth: ''
  ...
  methods: {
    ...
    saveCheckList(panelId, visible) {
      ...
      // var item = this.checkList.find(item => item.panelId == panelId)
      // item.visible = visible
      // this.checkList = Object.assign({}, this.checkList, item)
    },
    chartCheckList(data) {
      this.checkList = data
    },
