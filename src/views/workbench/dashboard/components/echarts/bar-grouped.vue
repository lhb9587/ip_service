<template>
  <div>
    <BarStackDerivative v-if="isAddStack&&isCustomer" :nameArr="nameArr" :data="data" :seriesData="seriesData" :chartData="chartData" />
...
<script>
import countIcon from '@/assets/u600.png' 
import stackIcon from '@/assets/stack.png' 
import ECharts from "vue-echarts";
import BarStackDerivative from './bar-stack-derivative.vue'
import Bus from "@/utils/Bus";
import { stateFormat } from "@/utils/filters";
export default {
  name: 'SbarGrouped',
  props: ['chartData', 'chartSelectName','isCustomer'],
  components: { "v-chart": ECharts, BarStackDerivative },
  data() {
    return { 
      dialog: false,
      title: '',
  ...
  methods: {
      ...
      this.nameArr && this.chartData.list.forEach((item, index) => {
        ...
        this.seriesData.push({
          label: {
            ...
            position: 'top',
            rotate: 30,
            offset: [4077].includes(this.chartData.panelId)? [0, -15]:[20,-15],
            z:10,
            formatter: (params) => {
              const result = this.stateFormat(params.value)
              return result
            }
          },
          stack:this.isStack && 'sum',
          name: item.legend,
    ...
    optionInit() {
      this.option = {
        ...
        toolbox: {
          ...
          feature: {
            ...
            myCustomButton: {
              ...
              show:true,
              title: '数字显示',
              icon: `image://${countIcon}`, // 按钮图标的 URL
              onclick: () => {
                // 点击按钮时的操作
                this.showCount = !this.showCount;
                (this.currentType =='bar'|| this.currentType =='line') && this.chartInit()
              },
              backgroundColor: '#fff',
              connectedBackgroundColor: '#fff'
            },
            myCustomButton2: this.isCustomer?{
              emphasis: {
                iconStyle: {
                  textPosition: 'left'
                }
              ...
              show:true,
              title: '衍生堆叠图',
              icon: `image://${stackIcon}`, // 按钮图标的 URL
              onclick: () => {
                // 点击按钮时的操作
                this.isAddStack = !this.isAddStack
              },
              backgroundColor: '#fff',
              connectedBackgroundColor: '#fff'
            }:{}
        ...
        series: [605].includes(this.chartData.panelId)
              ...
              formatter: (params) => {
                if (this.isStack) {
                  if (this.seriesData.length -1 == index) {
                    ...
                    return  this.stateFormat(total)
                  }else {
                    return ''
                  }
                }else{
                  return  this.stateFormat(params.value)
                }
              }
            }
          }))
