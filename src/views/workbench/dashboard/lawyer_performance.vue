      ...
      <div class="header-right" style="cursor:pointer;">
        <div class="right-desc">
          ...
          >
            ...
            <div>
              记录工时合计=案内记录工时+案外记录工时<br />
              核定工时合计=案内核定工时+案外核定工时<br />
              案内工时转化率=案内核销工时/案内核定工时<br />
              案内工时核定率=案内核定工时/案内记录工时
            </div>
          </el-popover>
        </div>
    ...
    <div class="main">
      <div class="main-list">
        <div class="hrm-search">
          <el-select v-model="jxType" placeholder="请选择" style="width: 120px;float: left" size="small" @change="changeRole">
            <el-option
              v-for="item in [{label: '员工', value: 0}, {label: '客户负责人', value: 1}]"
              :key="item.value"
              :label="item.label"
              :value="item.value">
            </el-option>
...
<!--          <el-button v-if="this.$store.getters.permissions.includes(385)" size="mini" style="margin-left: 50px" @click="performanceVisible = true">绩效规则</el-button>-->
          <div style="float: right">
            <el-button type="text" @click="deriveList()">导出数据</el-button>
          </div>
        </div>
        <section>
          <div class="w-table">
...
<!--            <el-button class="brush_right" :class="brushRightHasFilter?'red':''" @click="brushRight" type="text" icon="el-icon-brush" ></el-button>-->
    ...
    <el-dialog title="律所绩效-计算规则配置" :visible.sync="performanceVisible" :close-on-click-modal="false" width="30%">
      <performanceRules v-if="performanceVisible" @closeDialog="performanceDialog"></performanceRules>
    </el-dialog>
  </div>
</template>

<script>
import { queryJixiao } from '@/api/dashboard.js'
...
import performanceRules from './components/performance/performanceRules'
export default {
  name: 'lawyer_performance',
  data() {
    var tableHeader=[]
    if(localStorage.getItem('tableHeader')&&JSON.parse(localStorage.getItem('tableHeader')).find(item=>item.name==(this.$options.name+this.jxType))){
      tableHeader = JSON.parse(localStorage.getItem('tableHeader')).find(item => item.name == (this.$options.name+this.jxType)).tableHeader
    }
    return {
      tableTitleList: [
        {'title': '工作组', 'value': 'groupName',  'flex': 1, 'cellClass': ''},
        {'title': '核销工时', 'value': 'hoursAmount',  'flex': 1, 'cellClass': ''},
        {'title': '核定工时', 'value': 'bossAmount',  'flex': 1,'cellClass': ''},
        {'title': '案外工时', 'value': 'noCaseAmount', 'flex': 1, 'cellClass': ''},
        {'title': '案量', 'value': 'caseCount', 'flex': 1, 'cellClass': ''},
      ],
      detailCellRendererParams: {
        ...
        getDetailRowData: params => {
          ...
          queryJixiao({
            ...
            createDateArray: this.queryModuleData.createDateArray
          }).then(res => {
            params.successCallback(res.data.sort((a, b) => b.zhlCount - a.zhlCount))
          })
        },
      },
      jxType: 0,
  ...
  methods: {
    ...
    // 导出数据
    deriveList() {
      deriveList({createDateArray:this.createDateArray,bussId: this.bussId, isBoss: this.jxType }).then(res=>{
    ...
    queryRecordList() {
      this.queryModuleData = JSON.parse(localStorage.getItem('queryModuleData')) ? JSON.parse(localStorage.getItem('queryModuleData')).find(item => item.name == (this.$options.name+this.jxType)) ? JSON.parse(localStorage.getItem('queryModuleData')).find(item => item.name == (this.$options.name+this.jxType)).queryModuleData : {} : {}
    ...
    // 右侧清理列过滤器
    brushRight(){
      let data = this.$commonUtils.getFilterModel((this.$options.name+this.jxType)+this.$route.name,'FilterModel')
    ...
    getFilterModel(){
      this.$nextTick(()=>{
        ...
        let sortData =   this.$commonUtils.getFilterModel((this.$options.name+this.jxType)+this.$route.name,'SortModel')
    ...
    cellRenderer(params) {
      // 是否显示展开事件
      if (params.colDef.field == 'userFullName' && !this.jxType) { // 员工
    ...
    refreshEvenRowsCurrencyData(params) {
      ...
      if(this.list.length){
        ...
        if(params.type=="filterChanged"){
          this.$commonUtils.saveFilterModel((this.$options.name+this.jxType)+this.$route.name,'FilterModel',this.gridApi.getFilterModel())
          var data = this.$commonUtils.getFilterModel((this.$options.name+this.jxType) + this.$route.name, 'FilterModel');
        ...
        if(params.type=="sortChanged"){
          this.$commonUtils.saveFilterModel((this.$options.name+this.jxType)+this.$route.name,'SortModel',this.gridApi.getSortModel())
        }
      }
    },
    // 搜索内容
    ...
    performanceDialog(status){
      this.performanceVisible = false
      status && this.handleSearch()
    },
    // 搜索
    handleSearch(val){
    ...
    getAllWfs() {
      ...
      Promise.all([selectColumn(data), queryPreference(data)]).then(res=>{
        ...
        // this.bussId 89 -> 员工， 87 -> 客户负责人
        // 员工：员工姓名，员工职位
        // 案内工时：记录工时，核定工时，核销工时，未核销工时，案量；案外记录工时：客户开发维护，其他，小计；案外核定工时：客户开发维护，其他，小计；合计：记录工时合计，核定工时合计；
        // 客户负责人：员工姓名
        // 案内工时：记录工时，核定工时，核销工时，未核销工时，案量，工时转化率；案外记录工时：客户开发维护，其他，小计；案外核定工时：客户开发维护，其他，小计；合计：记录工时合计，核定工时合计；
        let list = []
        if (this.bussId === 89) {
          this.preferenceList.forEach(item => {
            if (['userFullName', 'positionStr'].includes(item.value)) {
              list.push({
                headerName: item.title,
                field: item.value,
                width:this.getLoactionWidth(item.title)=='auto'?250:this.getLoactionWidth(item.title),
                resizable: true,
                // flex: 1,
                sortable: true,
                filter: 'agSetColumnFilter',
          ...
          this.preferenceList.forEach(item => {
            if (['userFullName'].includes(item.value)) {
              list.push({
                headerName: item.title,
                field: item.value,
                width:this.getLoactionWidth(item.title)=='auto'?250:this.getLoactionWidth(item.title),
        ...
        list.forEach(item => {
          if (item.children) {
            item.children = item.children.map(j => {
              let obj = {}
              obj = this.preferenceList.find(k => k.value === j.value)
              return {
                headerName: obj.title,
                field: obj.value,
                width:this.getLoactionWidth(obj.title)=='auto'?250:this.getLoactionWidth(obj.title),
                resizable: true,
                // flex: 1,
                sortable: true,
                filter: 'agSetColumnFilter',
                ...
                comparator: ['hedinglv', 'zhuanhualv'].includes(obj.value) ? (a, b) => {
                  if (!a && !b) {
                    return 0
                  }
                  if (!a) {
                  ...
                  if (Number(a) + '' !== 'NaN') {
                    return a - b
                  } else if (typeof a == 'string' && a.includes('%') && Number(a.replace(/%/, '')) + '' !== 'NaN') {
                  ...
                  return this.$commonUtils.sort(a,b,obj.value)
                }
              }
            })
          }
        ...
        this.columnDefs.forEach(item=>{
          ...
          } else if (item.field == 'zhuanhualv') { // 针对百分比定义排序规则排序
            item.comparator = (a, b) => {
              if (!a && !b) {
                return 0
              }
    ...
    saveLocation(){
      if(!localStorage.getItem('tableHeader')){
        let arr=[]
        arr.push({name:(this.$options.name+this.jxType),tableHeader:this.tableHeader})
        localStorage.setItem('tableHeader',JSON.stringify(arr))
      }else {
        let arr=JSON.parse(localStorage.getItem('tableHeader'))
        if(arr.find(item=>item.name==(this.$options.name+this.jxType))){
          arr.find(item=>item.name==(this.$options.name+this.jxType)).tableHeader= this.tableHeader
          localStorage.setItem('tableHeader',JSON.stringify(arr))
        } else {
          arr.push({name:(this.$options.name+this.jxType),tableHeader:this.tableHeader})
          localStorage.setItem('tableHeader',JSON.stringify(arr))
    ...
    queryList(){
      ...
      querData.isBoss = this.jxType

      queryJixiao(querData).then(res=>{
        if (res.data){
          this.total=res.total
          this.listLoading = false
          this.list= res.data.sort((a, b) => b.zhlCount - a.zhlCount)
          if (this.userList.length <= 0){
            this.userList = res.data.sort((a, b) => b.zhlCount - a.zhlCount)
          }
        } else {
          this.$message.error('绩效规则配置错误')
        }
      })
    },
    confirmExactButton(){
    ...
    headerDragend(newWidth, oldWidth, column, event){
      ...
      var header=arr.find(item=>item.name==(this.$options.name+this.jxType)).tableHeader
  ...
  watch:{
    ...
    tableHeaderLabelList(n,o){
      if(o.length){
        if(localStorage.getItem('tableHeader')){
          let header=JSON.parse(localStorage.getItem('tableHeader')).find(item=>item.name==(this.$options.name+this.jxType)).tableHeader
