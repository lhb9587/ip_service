<template>
  <div>
    ...
    >
      ...
      <el-table-column property="docTypeName" label="文件分类"
                       :width="fileTableHeader.docTypeName || 'auto'"></el-table-column>
      <el-table-column label="文件类型" :width="fileTableHeader.typeName || 'auto'">
        <template slot-scope="scope">
          <span>{{scope.row.typeName}}</span>
          <!--            <span-->
          <!--              style="cursor:pointer;">{{-->
          <!--              scope.row.isDir == 1 ? '文件夹' : scope.row.fileName&&scope.row.fileName.substring(scope.row.fileName.lastIndexOf('.') + 1)-->
          <!--            }}</span>-->
        </template>
      </el-table-column>
      <el-table-column property="nwTypeName" label="文件归类" :width="fileTableHeader.nwTypeName || 'auto'">
        <template slot-scope="scope">
          <span :title="scope.row.nwTypeName">{{scope.row.nwTypeName}}</span>
        </template>
      </el-table-column>
      <el-table-column property="fileName" label="附件名称" :width="fileTableHeader.fileName || 'auto'">
        <template slot-scope="scope">
          <span :title="scope.row.fileName" class="pointer" @click="preView(scope.row)">{{scope.row.fileName}}</span>
        </template>
      </el-table-column>
      <el-table-column property="applicantName" label="关键字" :width="fileTableHeader.keywordStr || 'auto'">
        <template slot-scope="scope">
            <span :title="scope.row.keywordStr">
              {{scope.row.keywordStr}}
            </span>
        </template>
      </el-table-column>
      <el-table-column property="nwTypeStr" label="案件类型"
                       :width="fileTableHeader.nwTypeStr || 'auto'"></el-table-column>
      <el-table-column property="fullname" label="外代所名称" :width="fileTableHeader.fullname || 'auto'">
        <template slot-scope="scope">
          <span :title="scope.row.fullname" class="pointer"
                @click="queryWDS(scope.row.custId)">{{scope.row.fullname}}</span>
        </template>
      </el-table-column>
      <el-table-column property="countryCn" label="国家/地区" :width="fileTableHeader.countryCn || 'auto'">
        <template slot-scope="scope">
          <span :title="scope.row.countryCn">{{scope.row.countryCn}}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import {
    copyNwMaterialAndDir,
    modifyDir,
    modifyNwMaterial, moveNwMaterialAndDir,
    queryNwMaterial,
    querySortPermission,
  ...
  export default {
    ...
    methods: {
      ...
      loadNode(node, resolve) {
        ...
        queryNwMaterial({sortId: node.data.sortId, pageSize: 9999}).then(res => {
          this.pathArr.push({
      ...
      getRowContextmenuList(row) {
        return [
          ...
          {name: '修改', action: 'editFun', permissions: 359, isDir: [0]},
          {name: '重命名', action: 'changeDirName', permissions: 359, isDir: [1]},
          {name: '复制', action: 'copyFile', permissions: 359, isDir: [0, 1]},
          {name: '移动', action: 'moveFile', permissions: 359, isDir: [0, 1]},
          {name: '删除', action: 'handledeleElement', permissions: 359, isDir: [0, 1]},
          {name: '分配权限', action: 'allocation', permissions: 394, isRoot: true, isDir: [1]},
        ].filter(item => {
          if (item.permissions) {
            if (item.name === '修改' || item.name === '移动' || item.name === '重命名' || item.name === '复制') {
              return row.permTypeList && row.permTypeList.includes(2)
            } else if (item.name === '删除') {
              return row.permTypeList && row.permTypeList.includes(3)
            } else if (item.name === '分配权限') {
              return (this.$route.query.sortId === '0' || this.$route.query.sortId === undefined) && this.$store.getters.permissions.includes(item.permissions)
            }
          } else {
            return true
          }
