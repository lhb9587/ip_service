...
import { querySalaryTongji, queryOrganizations, salaryTongjiDeriveList } from '@/api/hrmList.js'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      listQuery: {
        pageNo: 0,
        pageSize:  JSON.parse(localStorage.getItem('pageSize'))?JSON.parse(localStorage.getItem('pageSize')).find(item=>item.name==this.$route.name)?JSON.parse(localStorage.getItem('pageSize')).find(item=>item.name==this.$route.name).pageSize:1000:1000
      },
      ...
      cascaderLable: '',
      yearRange: [],
      defaultYearColumnDefs: [
        { headerName: '工资标准', field: 'wageStandard' },
        { headerName: '年应发工资', field: 'payable' },
        { headerName: '月奖标准', field: 'bonusStandard' },
        { headerName: '年应发月奖', field: 'bonus' },
        { headerName: '社保（公司）', field: 'retirement' },
        { headerName: '公积金（公司）', field: 'fundInc' },
        { headerName: '年中奖', field: 'winningAmount' },
        { headerName: '年终奖', field: 'bonusAmount' },
        { headerName: '全年收入（含社保公积）', field: 'totalIncome' },
        { headerName: '全年收入（不含社保公积）', field: 'income' },
        { headerName: '年终高合', field: 'gaoheAmount' },
        { headerName: '年终打分', field: 'scoring' },
        { headerName: '备注', field: 'memo' },
        { headerName: '调薪', field: 'adjustRecord' }
      ],
      defaultYearTemplate: {
        headerName: '',
        headerClass: '',
      ...
      yearListcolumnDefs: [], // 年份列
      computeColumnDefs: [], // 要计算列定义
      empStatus: '', // 查询工作组在职条件
    }
  },
  created() {
  ...
  methods: {
    ...
    autoSizeAll() {
      const allColumnIds = [];
      this.gridApi.columnController.gridColumns.forEach((column) => {
        allColumnIds.push(column.getId());
      });
      this.gridApi.columnController.columnApi.autoSizeColumns(allColumnIds,false);
    },
    // 获取工作组列表
    queryOrganizations(){
      var empStatus = this.queryModuleData ? ((this.queryModuleData.empStatus && this.queryModuleData.empStatus == '在职') ? 1 : 0) : 0
      if (empStatus !== this.empStatus){
        queryOrganizations({ status: empStatus }).then(res => {
          if (res.success) {
    ...
    calculateYearRange() {
      ...
      for (let i = currentYear; i >= currentYear - 3; i--) {
        ...
        num = num - 1;
        const template = JSON.parse(JSON.stringify(this.defaultYearTemplate))
        template.headerName = i
        template.headerClass = 'my-css-class' + num
        template.children.forEach(item => {
          if(!['scoring','memo','adjustRecord'].includes(item.field)){
            this.computeColumnDefs.push(item.field + num)
          }
          item.field = item.field + num
          item.headerClass = 'my-css-class' + num 
        ...
        this.yearListcolumnDefs.push(template)
      }
      this.autoSizeAll()
    },
    // 附件下载
    ...
    exportList(flag) {
      let queryData = {}
      if (flag === 2) {
        let idList = this.getCurrentRowsData('talentCode')
        queryData = Object.assign(JSON.parse(JSON.stringify(this.queryModuleData)), { bussId: this.bussId, idArray: idList })
      } else {
        queryData = Object.assign(JSON.parse(JSON.stringify(this.queryModuleData)), { bussId: this.bussId })
      }
      salaryTongjiDeriveList(this.$commonUtils.cleanNullAttr(queryData)).then(res=>{
    ...
    queryRecordList() {
      ...
      this.queryModuleData.deptCode = this.deptCodes ? this.deptCodes[this.deptCodes.length-1] : ''
    ...
    cellRenderer(params) {
      ...
      }else if(this.computeColumnDefs.includes(params.colDef.field)){
    ...
    getAllWfs() {
      ...
      Promise.all([selectColumn(data), queryPreference(data)]).then(res=>{
        ...
        // 置空年份字段模板
        this.defaultYearTemplate.children = []
        this.yearListcolumnDefs = []
        const updatedPreference = this.preferenceList.filter(item => {
          const foundColumn = this.defaultYearColumnDefs.find(col => col.field === item.value);
          if (foundColumn) {
            this.defaultYearTemplate.children.push(foundColumn);
            return false; // 过滤掉匹配的项
          }
          return true; // 保留不匹配的项
        });
        ...
        this.columnDefs=[...this.defaultcolumnDefs,...updatedPreference.map(item=>({
        ...
        this.$nextTick(() => {
          ...
          if(this.defaultYearTemplate.children.length > 0){
            this.calculateYearRange()
            this.columnDefs = this.columnDefs.concat(this.yearListcolumnDefs);
    ...
    queryList(){
      ...
      querySalaryTongji(this.$commonUtils.cleanNullAttr(querData)).then(res=>{
  ...
  computed:{
    ...
    pinnedBottomRowData() {
      if (this.gridApi.rowModel) {
        return this.$commonUtils.pinnedBottomRowData(this.gridApi.rowModel.rootNode.childrenAfterFilter.map(item=>item.data), this.columnDefs, this.computeColumnDefs)
      }
      return ''
    }
  },
