...
<!--        :options="groupList"-->
<!--        :props="{value : 'deptCode', label: 'kingDeeGroupName', children: 'children'}"-->
<!--        multiple-->
<!--        filterable-->
<!--        clearable-->
<!--        change-on-select-->
...
<!--        v-model="deptCodes"-->
<!--        placeholder="部门工作组"-->
<!--        @active-item-change="activeItemChange"> </el-multi-cascader>-->
      ...
      <div style="float: right">
        <template v-if="$store.getters.permissions.includes(363)">
          <el-button type="text" @click="exportPersonInfoList()" >导出多行信息模板</el-button>
          <span style="color: #52A2F4;margin: 0 5px"> / </span>
        </template>
        <template v-if="$store.getters.permissions.includes(365)">
          <el-button type="text" @click="salaryTongjiDeriveList()" >导出调薪记录</el-button>
          <span style="color: #52A2F4;margin: 0 5px"> / </span>
        </template>
    ...
    <SelectOption
      ...
      @cancel="closeSelect"
      :idArray="getCurrentRowsData('talentCode')"
    ...
    >
      <AssessContent v-if="assessVisible" :talent-code="assessTalentCode" @clearAssess="assessVisible = false"></AssessContent>
    </el-dialog>

    <el-dialog
      title="设置考核内容-2"
      :visible.sync="assessTwoVisible"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
      :fullscreen="true"
      width="80%"
      @close="assessTwoVisible = false"
    >
...
import {queryLoanList, selectColumn, queryPreference, delPreference, savePreference} from '@/api/caseList.js'
import { queryTalentPersonList, queryPersonAll, queryPositions, queryOrganizations, exportPersonInfoListToExcel, salaryTongjiDeriveList } from '@/api/hrmList.js'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      deptCodes: [],
      positionName: '',
      empStatus: '在职', // 在职状态 0=>离职员工，1=>正式员工，2=>实习员工，3=>兼职人员，4=>试用人员，5=>退休返聘
      userList: [], // 用户列表
      groupList: [], // 工作组列表
      positionsList: [],// 岗位列表
      // statusList: [
      ...
      defaultcolumnDefs:[
        {
          ...
          field: 'sid',
          width: 80,
          enableRowGroup: true, // 设置为false，可隐藏顶部行组面板
          // enablePivot: true,
  ...
  methods: {
    ...
    // 导出人员多行信息
    exportPersonInfoList() {
      exportPersonInfoListToExcel(this.queryModuleData).then(res => {
        this.downLoad(res.data)
      })
    },
    // 导出人员调薪记录
    salaryTongjiDeriveList() {
      let idArray = this.getCurrentRowsData('talentCode')
      salaryTongjiDeriveList({ bussId: 206, idArray: idArray }).then(res => {
    ...
    queryRecordList() {
      ...
      this.queryModuleData.talentCode = this.talentCode
      this.queryModuleData.posId = this.positionsList.find(item => item.posName == this.positionName) ? this.positionsList.find(item => item.posName == this.positionName).id : ''
      this.queryModuleData.empStatus = this.empStatus
    ...
    rowDoubleClicked(params){
      console.log(params.data.talentCode)
    ...
    getRowContextmenuList(row){
      ...
      list = [
        { name: '复制', action: 'copy' },
        { name: '设置考核内容', action: 'setAssessContent' }
      ]
    ...
    activeItemChange(val) {
      if (val[0] && this.activeDeptCode != val[0] && this.deptCodes) {
        var deptCodes = []
        this.deptCodes.forEach(item=>{
          if (item[0] === val[0]){}
          deptCodes.push(item)
        })
        this.deptCodes = JSON.parse(JSON.stringify(deptCodes))
        this.$refs.multiCascader.aa()
        this.activeDeptCode = val[0]
        // this.$forceUpdate()
