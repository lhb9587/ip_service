<template>
  <div>
    ...
    <el-upload
      ...
      multiple
      name="uploadFile"
      :action="upsertWinningBonusFromExcel"
      :data="{tokenID:$store.getters.token,bonusYear : bonusYear, bonusMonth : bonusMonth}"
      :file-list="fileList"
...
<script>
import { upsertWinningBonusFromExcel } from "@/api/ipServiceApi.config.js";
export default {
  name: 'MidYearBonus',
  data() {
    return {
  ...
  methods: {
    getLastMonthYearMonth() {
      ...
      this.bonusYear = lastMonthYear;
      this.bonusMonth = lastMonthMonth;
  ...
  watch: {
    years(newYears) {
      ...
      this.bonusYear = matches ? matches[0] : '';
      this.bonusMonth = matches ? matches[1] : '';
    },
  }
}
</script>
