  ...
  <div class="main-list">
    <div class="hrm-search">
      ...
      <div style="display: inline;margin-left: 10px;">
        <el-button type="primary" size="small" plain @click="addExtInfo">添加备注信息</el-button>
      </div>
    ...
    <el-dialog title="备注信息" :visible.sync="salaryConfigVisible" :close-on-click-modal="false" width="30%">
      <ExtInfoSave v-if="salaryConfigVisible" :salary-config-data="salaryConfigData" @closeDialog="salaryConfigDialog"></ExtInfoSave>
    </el-dialog>
...
import { queryTalentPersonExtInfo, deleteTalentPersonExtInfo } from '@/api/hrmList.js'
import Summary from "@/components/Summary/index.js";
import headerSearch from '@/components/HeaderSearch';
import ExtInfoSave from "./components/ExtInfoSave"
export default {
  name: 'extInfoList',
  data() {
    var tableHeader=[]
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('此操作删除备注信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteTalentPersonExtInfo({ idList: extIds }).then(res=>{
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
    ...
    queryList(){
      ...
      queryTalentPersonExtInfo(this.$commonUtils.cleanNullAttr(querData)).then(res=>{
