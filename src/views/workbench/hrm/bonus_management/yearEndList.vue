  ...
  <div class="main-list">
    <div class="hrm-search">
      ...
      <div style="display: inline;margin-left: 10px;">
        <el-button type="primary" size="small" plain @click="addYearEndInfo">添加年终奖金</el-button>
      </div>
    ...
    <el-dialog title="年终奖信息" :visible.sync="saveVisible" :close-on-click-modal="false" width="30%">
      <YearEndBonusSave v-if="saveVisible" :data-info="dataInfo" @closeDialog="saveDialog"></YearEndBonusSave>
    </el-dialog>
  </div>
</template>
...
import { queryTalentBonusInfo, deleteTalentBonusInfo } from '@/api/hrmList.js'
import Summary from "@/components/Summary/index.js";
import headerSearch from '@/components/HeaderSearch';
import YearEndBonusSave from './components/YearEndBonusSave';
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('此操作删除年终奖信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteTalentBonusInfo({ idList: ids }).then(res=>{
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
    ...
    queryList(){
      ...
      queryTalentBonusInfo(this.$commonUtils.cleanNullAttr(querData)).then(res=>{
