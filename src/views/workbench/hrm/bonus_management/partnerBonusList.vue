  ...
  <div class="main-list">
    <div class="hrm-search">
      ...
      <div style="display: inline;margin-left: 10px;">
        <el-button type="primary" size="small" plain @click="addExtInfo">添加合伙人奖金</el-button>
      </div>
    ...
    <el-dialog title="合伙人奖金" :visible.sync="salaryConfigVisible" :close-on-click-modal="false" width="30%">
      <PartnerBonusSave v-if="salaryConfigVisible" :salary-config-data="salaryConfigData" @closeDialog="salaryConfigDialog"></PartnerBonusSave>
    </el-dialog>
...
import { queryPartnerBonusInfo, deletePartnerBonusInfo } from '@/api/hrmList.js'
import Summary from "@/components/Summary/index.js";
import headerSearch from '@/components/HeaderSearch';
import PartnerBonusSave from "./components/PartnerBonusSave"
import {formatAmount} from '@/utils/filters'
export default {
  name: 'partnerBonusList',
  data() {
    var tableHeader=[]
  ...
  methods: {
    addExtInfo() {
      this.salaryConfigData = []
      this.salaryConfigVisible = true
    },
    queryRecordList() {
    ...
    salaryConfigDialog(){
      this.salaryConfigVisible = false
      this.handleSearch()
    },
    // 附件下载
    downLoad(data) {
    ...
    handleDelete(row) {
      ...
      if (row && row.id){
        ...
        extIds = list.map(item => item.id).join(',')
      }
      this.$confirm('此操作删除合伙人奖金, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deletePartnerBonusInfo({ idList: extIds }).then(res=>{
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
    ...
    edit(row) {
      this.salaryConfigVisible = true
      this.salaryConfigData = row
      console.log('aa',this.salaryConfigData)
    },
    ...
    queryList(){
      ...
      queryPartnerBonusInfo(this.$commonUtils.cleanNullAttr(querData)).then(res=>{
