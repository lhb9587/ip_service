<template>
  <div class="main-list">
    ...
    <el-dialog title="月工资信息" :visible.sync="saveVisible" :close-on-click-modal="false" width="70%">
      <SalaryInfoSave v-if="saveVisible" :data-info="dataInfo" @closeDialog="saveDialog"></SalaryInfoSave>
    </el-dialog>
...
import { queryTalentPersonSalaryInfo, deleteTalentPersonSalaryInfo, salaryTongjiDeriveList } from '@/api/hrmList.js'
import Summary from "@/components/Summary/index.js";
import headerSearch from '@/components/HeaderSearch';
import SalaryInfoSave from './components/SalaryInfoSave';
...
export default {
  ...
  data() {
    ...
    return {
      ...
      dataInfo: [],
      saveVisible: false,
      deleteFields: ['salaryYear','salaryMonth','talentCode','talentName','kingDeeGroupName','paymentPlace','taxRate','deleteFlag','createDate','modifyDate','optUserId','adjustRecord']
    }
  },
  created() {
    this.getAllWfs()
  ...
  methods: {
    ...
    exportList(flag) {
      let queryData = {}
      if (flag === 2) {
        let idList = this.getCurrentRowsData('slaId')
    ...
    cellRenderer(params) {
      ...
      }else if (!this.deleteFields.includes(params.colDef.field)) {
    ...
    handleDelete(row) {
      ...
      if (row && row.slaId){
        ...
        slaIds = list.map(item => item.slaId).join(',')
      }
      this.$confirm('此操作删除月工资信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteTalentPersonSalaryInfo({ idList: slaIds }).then(res=>{
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
    ...
    queryList(){
      ...
      queryTalentPersonSalaryInfo(this.$commonUtils.cleanNullAttr(querData)).then(res=>{
  ...
  computed:{
    ...
    pinnedBottomRowData() {
      if (this.gridApi.rowModel) {
        const preservedColumns = this.selectColumnList
        .filter(column => !this.deleteFields.includes(column.COLUMN_NAME))
        .map(column => column.COLUMN_NAME);
        return this.$commonUtils.pinnedBottomRowData(this.gridApi.rowModel.rootNode.childrenAfterFilter.map(item=>item.data), this.columnDefs, preservedColumns)
      }
      return ''
    },
  },
