...
<!--      <el-menu class="el-menu-demo" mode="horizontal" @select="handleSelect" v-allow="83" style="backgroundColor: #fff; border: none">-->
...
<!--    </div>-->
    ...
    <div class="content_wrap" style="margin-top: 10px">
      ...
      <div class="right_wrap" v-if="pattern" :style="{position:'relative','margin-left':currentCaseId&&pattern?'10px' :'0',height:clientHeight+'px'}">
        ...
        <seeCaseDetail v-if="currentCaseId" :case-id="currentCaseId"  :caseList="caseList" :sign="1" :backsign="0" :task-type="taskType">
        </seeCaseDetail>
      </div>
    </div>
...
<!--    </el-dialog>-->
    ...
    >
      <billDetail v-if="billDetailState" :type="type" :billData="billData" @cancel="cancel"></billDetail>
    </el-drawer>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
      ...
      if(billId){
        fetchList({ billId}).then(res => {
          if(res.data.length) {
            ...
            if(obj){
              this.$store.commit('tabs/CHANGE_STORAGETABLE',{type:'update',title:'暂存账单',name:'暂存账单',identification:this.identification,data:res.data[0]})
              console.log(this.list);
              this.$emit('cancel',res.data[0])
              // for (var key in obj) {
              //   if(key!='sid'){
    ...
    return [
      ...
      { name:'确认比例',action:'toExamine', permissions: 243, taskId:true,taskNo: 6},
      { name: '修改', action: 'editBill',permissions:163},
      { name: '删除', action: 'handledeleElement',permissions:164, }].filter(item=> {
      if (item.permissions) {
          if(item.taskId){
            if(row.taskList){
    ...
    // hidden(row){
    //     this.bList=this.bList.filter(item=>item[this.getCurFilterId(this.pageTitle)]!=row[this.getCurFilterId(this.pageTitle)])
    ...
    //       })
    //         .then(() => {
    //           deleteBillUrl({ billId: item.billId }).then(response => {
    //             if (response.success) {
...
/deep/ .ag-root-wrapper.ag-layout-normal {
  height: 95%;
}
.queryButton {
  display: flex;
