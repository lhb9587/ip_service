...
<!--      <el-menu class="el-menu-demo" mode="horizontal" @select="handleSelect" v-allow="83" style="backgroundColor: #fff; border: none; height: 75px" >-->
...
import { queryCustomerNameId, selectColumn, queryPreference, savePreference,configFilter ,queryFilterConfig,queryFilter,addFilterScheme,delPreference,queryApplicantByIdList,checkPermission} from "@/api/caseList.js";
  ...
  methods: {
    ...
    getRowContextmenuList(row){
      return [
        ...
        { name: '删除', action: 'deleteFee',permissions:223, }].filter(item=> {
        if (item.permissions) {
    ...
    takeCaseDetail(row) {
      if(row.taskType){
        this.$router.push({ path: '/workbench/case/seeCaseDetail/' + row.caseId, query: {taskType:row.taskType}})
      }else {
        this.$router.push({ path: '/workbench/case/seeCaseDetail/' + row.caseId, query: {taskType:1}})
      }
    ...
    // hidden(row){
    //     this.cList=this.cList.filter(item=>item[this.getCurFilterId(this.pageTitle)]!=row[this.getCurFilterId(this.pageTitle)])
