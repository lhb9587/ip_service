<template>
  <div class="trademark-writing-wrapper">
    ...
    <div class="btn-wrapper fl-ac-jc">
      ...
      <el-button size="small" type="primary" @click="doWriteTask(2)" v-if="$store.getters.permissions.includes(380)">
        撰写提交审核
      </el-button>
      <el-button disabled size="small" type="primary" @click="doWriteTask(3)"
                 v-if="$store.getters.permissions.includes(381)">负责人审核(暂跳过)
      </el-button>
      <el-button size="small" type="primary" @click="doWriteTask(4)" v-if="$store.getters.permissions.includes(382)">
        客户组审核
      </el-button>
        ...
        <el-form label-width="120px" @submit.native.prevent size="mini" @keyup.enter.native="handleSearch(1)"
          ...
          <div style="display: flex;justify-content: left">
            <el-form-item label="撰写状态:" style="margin-bottom: 0!important">
              <el-radio-group v-model="queryModuleData.atStatus"
                              @change="() => handleSearch(1)">
                <el-radio :label="1">待指派撰写人</el-radio>
                <el-radio :label="2">待撰写提交审核</el-radio>
                <el-radio :label="3" disabled>负责人审核</el-radio>
                <el-radio :label="4">客户组审核</el-radio>
              </el-radio-group>
            </el-form-item>
            <!--            <el-form-item label="快捷查询:" style="margin-bottom: 0">-->
            <!--              <el-input type="text" v-model="queryModuleData.regAndAgent" clearable-->
            <!--                        @change="(e)=>handleSearch(1)"></el-input>-->
            <!--            </el-form-item>-->
          </div>
        </el-form>
      </div>
    ...
    <div class="content_wrap">
      ...
      <div class="right_wrap"
        ...
        <seeCaseDetail :style="cssVar" v-if="currentCaseId&&pattern" :case-id="currentCaseId" :taskType="taskType"
                       :sign="1" :backsign="0">
        </seeCaseDetail>
    ...
    >
      <trademark-writing-detail v-if="trademarkWritingView" :atData="atData" :type="type" :taskNo="taskNo"
                                ...
                                :caseIds="trademarkWritingCaseIdList"
  ...
  export default {
    ...
    methods: {
      ...
      closetrademarkWriting(flag) {
        this.$set(this.atData, 'isModifyFlag', false)
        this.$set(this.atData, 'editWritePeople', 0)
        this.editWritePeople = 0
      ...
      async doWriteTask(type) {
        ...
        switch (type) {
          ...
          case 1:
            ...
            const taskNoList = taskList.map(item => item.taskNo)
            if (taskNoList.find(item => item != 1)) {
              this.$message.error('所选任务不符合流程!')
              return
            }
            const selectedTaskIdList = taskList.map(item => item.taskId)
            if ([...new Set(selectedTaskIdList)].length > 1) {
              this.$message.error('不能同时指派多个任务!')
              return
            }
            ...
            this.taskIdList = taskList.map(item => item.taskId)
            this.taskNo = taskList.map(item => item.taskNo)[0]
            this.atData = this.getSelectedRows()[0]
            this.type = 'edit'
            this.title = '编辑'
            this.trademarkWritingView = true
            // this.trademarkWritingCaseIdList = [...new Set(this.getSelectedRows().map(item => item.caseId))]
            break
          case 2:
            taskList = this.getTaskList()
            if (taskList && !taskList.length) {
            ...
            if (this.getSelectedRows().length === 1) {
              ...
              this.taskNo = taskList.map(item => item.taskNo)[0]
              if (type != this.taskNo) {
                this.$message.error(`选中的商标撰写不符合该流程`)
                return
              }
              const res1 = await checkPermission({
                articleCaseList: this.getSelectedRows().map(item => ({
                  atId: item.atId,
                  caseId: item.caseId
                }))
              })
              if (res1 && res1.data) {
                this.editWritePeople = res1.data.zhuanxieren // 是否可以修改撰写人
              }
              this.atData = this.getSelectedRows()[0]
              ...
              this.$confirm('撰写提交审核', '提示', {
                confirmButtonText: '撰写完成',
                cancelButtonText: '取消',
                type: 'warning'
              }).then(() => {
                doArticleTask({
                  taskIdList: taskList.map(item => item.taskId)
                }).then(res => {
                  this.queryList()
                })
              })
          ...
          case 4:
            ...
            if (this.getSelectedRows().length === 1) {
              ...
              const list = [...new Set(this.getSelectedRows().map(j => j.complStr))]
              const params = {
                compl: list.length > 1 ? '' : (list[0] === '否' ? 2 : list[0] === '是' ? 1 : ''),
              }
                ...
                template: `
                  <div>
                    <el-form label-width="160px">
                      <el-row>
                        <el-form-item style="margin-bottom: 0" label="首次递交理由是否完整:">
                          <el-radio-group v-model="compl">
                            <el-radio :label="1">是</el-radio>
                            <el-radio :label="2">否</el-radio>
                          </el-radio-group>
                        </el-form-item>
                        <div style="color: #E6A23C" v-if="list.length > 1">注意： 所选撰写中【首次递交理由是否完整】数值不统一，选择以统一数值！</div>
                      </el-row>
                    </el-form>
                  </div>`,
                props: {
                ...
                beforeClose: (action, instance, done) => {
                  if (action === 'close') {
                    ...
                    if (params.compl) {
                      ...
                      this.$message.error('请选择首次递交理由是否完整')
                    }
                  }
                }
              }).then(() => {
            ...
            break
          default:
            if ([...new Set(this.getSelectedRows().map(item => item.atId))].length > 1) {
              this.$message.error('不能同时操作多个指派任务!')
              return
            }
            taskList = this.getTaskList()
            if (taskList && !taskList.length) {
      ...
      queryList(data = {}) {
        ...
        queryArticleList(reqData).then(res => {
