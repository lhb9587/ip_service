...
import { queryCustomerNameId ,sendBusiness,getBusinessList} from '@/api/caseList'
...
export default {
  ...
  data(){
    return {
      ...
      tableTitleList: [
        {"title":"客户名称","value":"custName" }, { 'title': '客户信用等级', 'value': 'custLevel',width:'100' },{"title":"邮件主题","value":"emailTheme" },
        {"title":"可续展商标数量","value":"extensionTmNum" },{"title":"处于宽展期商标数量","value":"graceTmNum" },{title:'送达工作组',value: 'groupName'},
        {title:'最新发送邮件时间',value: 'sendDate'}],
      tableTitleList2:[
       {'title':'商标号','value':'regNumber'},{'title':'商标名称','value':'tmName'},{"title":"商标图样","value":"imgFilePath" },
        {"title":"商标类别","value":"tmGroup" },{"title":"客户","value":"custName" },{"title":"申请人","value":"applicantName" },{"title":"有效期起始日","value":"validStartDate" },{"title":"有效期截至日","value":"validEndDate" },
        {"title":"是否处于宽展期","value":"isGrace" },{"title":"处理结果","value":"disposeResult" },{"title":"发送记录","value":"RemindRecordDetail" }],
      selectData:[],
  ...
  methods:{
    ...
    getRowContextmenuListFather(row){
      return [{ name:'复制',action: 'copy' },{ name:'立案',action: 'createCaseFather' },{ name:'发送',action: 'sendFather' },]
    },
    ...
    getBusinessList(){
      getBusinessList(this.$commonUtils.cleanNullAttr(Object.assign(this.queryModuleData,this.listQuery))).then(res=>{
