          ...
          <el-col class="form-con-item" :span="24">
            ...
            <el-row class="form-border">
              <el-col :span="24">
                ...
                <el-row :class="bustypeList.includes('其他（请详述）') ? '' : 'fullRow'">
                  <el-col :span="bustypeList.includes('其他（请详述）') ? 12 : 24">
                    <el-form-item label="业务领域:" class="postInfo-container-item">

                      <span v-if="isLook">{{bustypeList.join('，')}}</span>
                      <el-select @change="changeBustype" default-first-option :clearable='true' placeholder="请选择" v-model="bustypeList" filterable multiple>
                        <el-option v-for="item in seletData.ywBussiness" :key="String(item.typeName)" :label="item.typeName" :value="String(item.typeName)">
                        </el-option>
                      </el-select>
                    </el-form-item>
                  </el-col>
                  <el-col v-if="bustypeList.includes('其他（请详述）')" :span="12">
                    <el-form-item label="详述:" class="postInfo-container-item">

                      <span v-if="isLook">{{bustypeList.join('，')}}</span>
                      <el-input v-else v-model="otherDesc"></el-input>
                    </el-form-item>
                  </el-col>
                </el-row>
                ...
                <el-row class="fullRow">
                  <el-col :span="24">
                    ...
                    <el-form-item label="联系人文档:" class="postInfo-container-item uploader-custfile">
                      <template>
                        <p v-if="isLook" v-for="item in dfconFromData.materialList" style="color: #409EFF">
                          <a target="_blank" :href="`ipdoc${item.address}`">{{item.name+item.format}}</a>
                        </p>

                        <el-upload v-if="!isLook" class="upload-demo" name="custContactFile" :data='custContactFiledata' ref="uploadcustContactFile" :action="custContactFileUrl" :before-upload="handlePreview"
                          :on-preview="preview" :on-remove="(file,fileList) => handleRemove(file,fileList)" :file-list="dfconFromData.materialList" :auto-upload="false">
...
import {
  ...
  queryCustomerAddrListUrl, delCustContactDoc
...
export default {
  ...
  data() {
    return {
      ...
      // select的获取id有对应文档查询
      seletDataArr: "1129,1042,1,1044,1053,1045,1076,1112,1063,1179",
      seletData: {
        dfkhdzlist: [],
        comLanguage: {}, //沟通语言
  ...
  methods: {
    ...
    fetchData(id) {
      queryCustomerContactUrl({ custContactId: id })
          .then(response => {
            this.dfconFromData = response.data;
            if (this.dfconFromData.bustypeList && this.dfconFromData.bustypeList.length>0) {
              this.dfconFromData.bustypeList.forEach((item,index) => {
                if (item.includes('&')) {
                  this.otherDesc = item.split('&')[1]
                  this.dfconFromData.bustypeList[index] = item.split('&')[0]
                }
              })
            }
            this.bustypeList = this.dfconFromData.bustypeList || []
    ...
    dficonSendSuccess(response, file, fileList) {
      if (response.success) {
        ...
        this.$set(this.dfconFromData, 'photo', URL.createObjectURL(file.raw))
        // this.dfconFromData.photo = '/ipdoc' + response.photo;
        // this.dfuserIconCust = URL.createObjectURL(file.raw);
        this.dfuserIconCust = '/ipdoc' + response.photo
        this.$message.success(response.message);
        console.log('+++++++++++++++++')
        console.log(this.dfconFromData.photo)
        console.log(this.dfuserIconCust)
        console.log('+++++++++++++++++')
      } else {
        this.$message.error(response.message);
    ...
    remotepaternalUnit(query) {
      if (query !== "") {
        queryCustomerNameId({ keyword: query, [this.pageType]: "1" }).then(
            response => {
              if (response.success) {
                this.seletData.paternalUnit = response.data;
              } else {
    ...
    async createCustAddr(type) {
      ...
      if (this.isdfItemTrue()) {
        ...
        if (this.bustypeList&&this.bustypeList.length>0) {
          if (this.otherDesc) {
            this.bustypeList.forEach((item,index) => {
              if (item === '其他（请详述）') {
                this.bustypeList[index] = item + '&' + this.otherDesc
              }
            })
          }
          tempData.bustypeList = this.bustypeList
        }else{
          tempData.bustypeList = []
        }
        ...
        case "dfcon":
          queryCustomerContactUrl({ custContactId: row.custContactId }).then(
              res => {
                ...
                if (this.dfconFromData.bustype && this.dfconFromData.bustype.includes('&')) {
                  this.otherDesc = this.dfconFromData.bustype.split('&')[1]
                  this.dfconFromData.bustype = this.dfconFromData.bustype.split('&')[0]
                }
                this.dfconFromData.status = +this.dfconFromData.status || ''
