<template>
  <div>
    <div class="topSearch">
      ...
      <el-input clearable style="width: 180px;margin-left: 10px" placeholder="输入监控主体查询" size="mini"
                @change="queryList(1)"
                v-model="queryData.monitorItem"
      >
      </el-input>
      <el-select :clearable="false" size="mini" style="margin-left: 10px" @change="queryList(1)" default-first-option
      ...
      <div class="fl-ac-jc">
        <el-select @change="queryList(1)" size="mini" style="margin-left: 10px;width: 120px" default-first-option
          ...
          <el-option
              v-for="item in [
            {label: '结果数量', value: 'mCount'},
              {label: '客户', value: 'custName'},
               {label: '监控主体', value: 'monitorItem'},
               ]"
              :key="item.value"
              :label="item.label"
              :value="item.value">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer fl-ac-jc">
        <el-button style="height: 28px;margin-left: 5px" type="success" round size="mini" @click="exportExcel">
          ...
          <el-button size="mini" style="margin-left: 15px" round type="success" @click="() => closeDialog(false)">
...
export default {
  ...
  data() {
    return {
      ...
      tableKeyOut: false,
      tableHeadFilters: {},
      supplierList: [{label: '白兔', value: 1}, {label: '社文', value: 2}, {label: '摩知轮', value: 3}, {label: 'IPS', value: 4},{label:'太原数世',value:5}],
      bottomInfo: {
        annNum: '',
        exitsResult: '',
        notExitsResult: ''
  ...
  methods: {
    ...
    closeDialog(isSearch) {
      ...
      if (!this.queryData.handle) {
        this.monitorList = this.monitorList.filter(item => item.whdId !== this.currentRowKey)
    ...
    exportExcel() {
      ...
      const list = this.$commonUtils.unique(this.resultSelectionList, 'whdId')
      const excelImportInfos = list.map(item => ({
        id: item.whdId,
        itemStr: this.resultSelectionList.filter(i => i.whdId == item.whdId).map(j => j.id).join(','),
        annNum: item.annNum
      }))
      downloadMonitorListExcel({
        excelImportInfos,
    ...
    queryList(flag) {
      ...
      queryoObjectHasResult(this.$commonUtils.cleanNullAttr({
        ...this.listQuery, ...this.queryData, trademarkMonitoringType: 2,
        state,
        handle,
        nullResult
      })).then(res => {
