<template>
  <div class="app-container">
    <div class="search-box">
      <div class="search-left">
        <div style="margin-right: 10px">
          <el-input ref="searchRef" :placeholder="$store.getters.permissions.includes(419) ? '申请人模糊检索' : '申请人检索(至少2个字)'" prefix-icon="el-icon-search" @change="(value) => {!value && vagueSearch('')}"
        ...
        <el-button-group class="button-group-search" v-allow="419">
          ...
          >
            <div style="position:relative;">
              <div class="searchWrap">
                <el-form size="mini" label-width="150px" class="form-container" @keyup.enter.native="handleSearch(0)"
            ...
            {{itmx}}
                      ...
                      <el-option v-for="(itm,idx) in item['valueList']" :key="itm.id" :label="itm.value||itm.value1"
                                 :value="itm.id">
                        <el-tooltip v-for="(itmx,key) in item['valueList'][idx]" v-if="key!='id'" class="item"
                                    effect="light" :content="itmx" placement="top">
                          <span>{{itmx}}</span>
              ...
              <el-button style="position:absolute;top: 0;right: 10px" type="text" size="small" @click="queryFilter"><i
        ...
        <el-button v-allow="52" type="primary" style="margin-left: 10px" size="small" icon="el-icon-edit" @click="createClick">创建</el-button>
      </div>
    </div>

    <TitleTotal :total="total" style="float: left"/>
    <div style="position: relative">
      ...
      <AgGridVue
        :style="{width:'100%',height: clientHeight+'px', 'marginTop': '30px'}"
    ...
    <el-dialog title="提示" :visible.sync="transferVisible" width="80%">
      <el-transfer v-model="transferValue" :key="transferData.filterId" :data="transferData" filterable
    ...
    <el-dialog
      ...
      :visible.sync="applicantView"
      width="90%"
      height="80%"
      class="abow_dialog"
      :before-close="() => closeDialog(false)"
    >
      <applicant-view ref="applicant" isCust v-if="applicantView && (titleType === '查看' || titleType === '审核' || titleType === '提交')" :app-id="applicantId"></applicant-view>
      <applicant-detail @closeDialog="closeDialog" isCust ref="applicant" :is-edit="titleType === '修改'" :app-id="applicantId" v-else></applicant-detail>
      <span slot="footer" class="dialog-footer fl-ac-jc">
        <el-button size="small" type="primary" v-if="titleType === '新建'" @click="createApplicant">创建</el-button>
        <el-button size="small" type="primary" v-if="titleType === '修改'" @click="updateApplicant">保存</el-button>
        <el-popover
          v-if="titleType === '审核'"
          placement="top"
          width="350"
        ...
        <el-button v-if="titleType === '提交'" type="primary" size="small" @click="submitClick">提交</el-button>
        <el-button style="margin: 0 10px"  size="small" @click="closeDialog(false)">取消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
<script>
  ...
  import { appExamine, appSubmit, delConflicterUrl } from '@/api/customerList'
  import SelectOption from '../../case/components/SelectOption'
  import ApplicantView from "../../customer_management/applicant/components/ApplicantView";
  ...
  export default {
    name: 'cust_applicant',
    components: {ApplicantDetail, ApplicantView, SelectOption, Pagination},
    props: {
      custId: {},
      custName: '',
      patent: {
    ...
    created() {
      // this.queryModuleData = JSON.parse(localStorage.getItem('queryModuleData')) // 全部搜索数据
    ...
    methods: {
      ...
      vagueSearch(e, forceSearch) {
        if (forceSearch) {
          if (this.$store.getters.permissions.includes(419)) {
            this.handleSearch(1)
          } else {
            e && e.length > 1 && this.handleSearch(1)
          }
        } else {
          if (!e) {
      ...
      createApplicant() {
        this.$refs.applicant.submitForm('postForm')
      },
      updateApplicant() {
        this.$refs.applicant.updatasubmitForm('postForm')
      },
      ...
      queryFilterConfig(type) {
        ...
        queryFilterConfig(data).then(res => {
          ...
          this.$nextTick(() => {
            ...
            if (type == 1) {
               // this.$store.getters.permissions.includes(419) && this.handleSearch(true)
            }
      ...
      getRowContextmenuList(row) {
        ...
        return [
          ...
          {name: '提交', action: 'submitFun'},
          {name: '审核', action: 'auditFun'},
          {name: '修改', action: 'editFun', permissions: 54},
          {name: '删除', action: 'handledeleElement', permissions: 55}
        ].filter(item => {
          if (item.name === '提交') {
            return row.review === 1 && (row.examineStatus === 1623 || row.examineStatus === 1624)
          } else if (item.name === '审核') {
            return row.review === 1 && row.examineStatus === 1622
      ...
      rowDoubleClicked(params) {
        this.applicantId = params.data.appId
        this.applicantView = true
        this.titleType = '查看'
        // if (this.patent) {
      ...
      async getList(type) {
        ...
        if (type) { // 外部
          data = {
            keywords: this.queryModuleData.keywords
          }
        } else { // 内部
        ...
        queryApplication(Object.assign(data, this.listQuery)).then(response => {
