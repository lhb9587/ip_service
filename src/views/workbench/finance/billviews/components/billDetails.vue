<template>
  <div class="createPost-container" id="createPost-container">
    <div class="" style="display: flex;flex-wrap:nowrap;width: 100%">
      <div class="zdjbxn">
        <el-button v-if="!custFlag" @click="arrowClick" class="arrow" type="text" icon="el-icon-d-arrow-right"> </el-button>
                          ...
                          <el-row class="">
                            <el-col :span="12">
                              <el-form-item label="案件文号:" class="postInfo-container-item">
                                ...
                                <el-button style="margin-left: 10px" size="mini" type="primary" v-if="$route.params.type!='view' && !createCommonBill" @click="changeCaseVisible=true">修改</el-button>
                                <ChooseCase
                          ...
                          <el-row v-if="data.taskType == 4">
                            <el-col :span="12">
                              <el-form-item label="账单类型:" class="postInfo-container-item">
                                <!-- <el-select v-if="$route.params.type!='view'"  :clearable="true"
                                           default-first-option filterable v-model="data.billStyle">
                                  <el-option v-for="(item,key) in billTypeList" :key="key"
                                             :label="item.billStyleName" :value="item.billStyle"></el-option>
                                </el-select> -->
                                <el-cascader
                            ...
                            <el-col :span="12">
                              <el-form-item label="代理人:" class="postInfo-container-item">
                                <el-select v-if="$route.params.type!='view'"  :clearable="true" @change="changeAgentUser"
                          ...
                          <el-row >
                            ...
                            <el-col :span="12">
                              <el-form-item label="所属工作组:" :prop="createCommonBill ? '' : 'wkgId'" class="postInfo-container-item">
                                <el-select v-if="$route.params.type!='view'" @change="checkWkgIsGj" :clearable="true" default-first-option filterable v-model="data.wkgId" placeholder>
                                  <el-option v-for="(item,key) in selectData.gzzs" :key="key" :label="item.groupName" :value="item.wkgId"></el-option>
                                </el-select>
...
<!--                              </template>-->
                          ...
                          <el-col :span="12">
                            <el-form-item label="客户文号:" class="postInfo-container-item">
                              <el-input :disabled="createCommonBill" type="text" v-model="data.custRefno" v-if="$route.params.type!='view'"></el-input>
                        ...
                        <el-row>
                          <el-col :span="12">
                            <el-form-item label="案件描述:" :prop="createCommonBill ? '' : 'caseDesc'" class="postInfo-container-item">
                              <div style="width: 100%;height: 70px;overflow: auto;font-size: 13px" id="caseDescShow" v-show="$route.params.type!='view' && !caseDescShow" @click="caseDescClick" v-html="caseDescHtml"></div>
                              <el-input ref="caseDescRef" @blur="caseDescBlur" :disabled="createCommonBill" v-show="$route.params.type!='view' && caseDescShow" size="small" v-model="data.caseDesc" type="textarea"
                                        :rows="3" style="width:100%"
                                        placeholder="模板语句：案件名称+案件个数  案件名称最多显示三条，超出以“等”表示，每个案件名称之间用“；”号隔开"></el-input>
                              <span v-if="$route.params.type == 'view'" v-html="getSpecialInfo(data.caseDesc)"></span>
                            </el-form-item>
                          </el-col>
                          <el-col :span="12">
                            <el-form-item label="客户其他账单信息:" class="postInfo-container-item">
                              <el-input  v-if="$route.params.type!='view'"  size="small" type="textarea" :rows="3" v-model="data.otherBillInfo" placeholder="请输入"></el-input>
                        ...
                        <el-row>
                          <el-col :span="12">
                            <el-form-item label="发生日期:" class="postInfo-container-item">
                                <el-date-picker :default-value="timeDefaultShow" v-if="$route.params.type!='view'" v-model="fwDatevalue" type="daterange" value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
                                <span v-else>{{data.startDate}}-{{data.endDate}}</span>
                            </el-form-item>
                          </el-col>
                          <el-col :span="12">
                            <el-form-item label="账单状态:" class="postInfo-container-item" v-if="$route.params.type=='view'">
                        ...
                        <el-row v-if="data.taskNo==4">
                          <el-col :span="12">
                            <el-form-item label="账期:" class="postInfo-container-item">
                              <el-date-picker :disabled="createCommonBill" v-if="$route.params.type!='view'" v-model="data.accountPeriod" type="date" value-format="yyyy-MM-dd"></el-date-picker>
                        ...
                        <el-row>
                          <el-col :span="12">
                            <el-form-item label="结算币种:" :prop="createCommonBill ? '' : 'curId'" class="postInfo-container-item">
                                <el-select v-if="$route.params.type!='view'"  default-first-option @change="bzchange" v-model="data.curId" placeholder>
                                  <el-option v-for="(item,key) in selectData.bzData" :key="key" :label="item.curName" :value="item.curId"></el-option>
                                </el-select>
                          ...
                          <el-col :span="12">
                            <el-form-item label="汇率:" class="postInfo-container-item">
                              <el-input v-if="$route.params.type!='view'" type="text" v-model="data.exchangeRate" ></el-input>
                        ...
                        <el-row>
                          <el-col :span="12">
                            <el-form-item label="账单联系人:" class="postInfo-container-item">
                              <template>
                                <el-select v-if="$route.params.type!='view'" multiple filterable  default-first-option v-model="data.custContactIdArray" placeholder clearable @change="changeCustContactIdArray">
                                  <el-option v-for="(item,key) in selectData.lxrData" :key="key" :label="`${item.called?item.called+' ':''}${item.name}`" :value="item.custContactId"></el-option>
                                </el-select>
<!--                                <el-autocomplete-->
<!--                                  v-if="$route.params.type!='view'"-->
<!--                                  class="inline-input"-->
...
<!--                                <span style="vertical-align: text-top;" class="itemTitle_style" v-if="$route.params.type!='view'"  @click="dialogVisibleTt=true">编辑联系人/抬头</span>-->
                              </template>
                            </el-form-item>
                          </el-col>
                          <el-col :span="12">
                            <el-form-item label="账单抬头:" class="postInfo-container-item">
                              ...
                              ></el-autocomplete>
                              <!--                              <el-select v-if="$route.params.type!='view'"  default-first-option v-model="data.addrId" placeholder>-->
                        ...
                        <el-row>
                          <el-col :span="12">
                            <el-form-item label="账单折扣:" class="postInfo-container-item">
                                ...
                                <el-input v-if="$route.params.type!='view'" size="small" type="number" @input="intValidator" min="1" max="100" v-model.number="data.discount" @change="discountChange" placeholder="请输入两位整数"/>
                          ...
                          <el-col :span="12">
                            ...
                            <el-form-item label="账单地址:" class="postInfo-container-item">
                              <el-autocomplete
                                ...
                                placeholder="请输入内容"
                              ></el-autocomplete>
                              <!--                                <el-select  v-if="$route.params.type!='view'" default-first-option v-model="data.billAddr" placeholder>-->
                        ...
                        <el-row>
                          <el-col :span="12">
                            <el-form-item label="电子邮件:" class="postInfo-container-item">
                                <el-input v-if="$route.params.type!='view'" size="small" v-model="data.billEmail" placeholder></el-input>
                          ...
                          <el-col :span="12">
                            <el-form-item label="邮寄地址:" class="postInfo-container-item">
                              <el-autocomplete
                                ...
                                placeholder="请输入内容"
                              ></el-autocomplete>
<!--                                <el-select  v-if="$route.params.type!='view'"  default-first-option v-model="data.addressMail" placeholder>-->
<!--                                  <el-option v-for="(item,key) in selectData.dzData.filter(itm=>!!itm.addressCn)" :key="key" :label="item.addressCn" :value="item.addressCn"></el-option>-->
<!--                                </el-select>-->
                        ...
                        <el-row>
                          ...
                          <el-col :span="12">
                            <el-form-item label="所属月:" class="postInfo-container-item">
                                <el-date-picker v-if="$route.params.type!='view'" @change="changeDate" :picker-options="pickerOptions" align="right" format="yyyy-MM-dd" placeholder="选择日期" type="date" v-model="data.belongToDate" value-format="yyyy-MM-dd" />
                        ...
                        <el-row>
                          ...
                          <el-col :span="12">
                            <el-form-item label="开单日期:" class="postInfo-container-item">
                                <el-date-picker v-if="$route.params.type!='view'" :picker-options="pickerOptions" align="right" format="yyyy-MM-dd" placeholder="选择日期" type="date" v-model="data.billDate" value-format="yyyy-MM-dd" />
                        ...
                        <el-row v-if="isGj">
                          <el-col :span="12" >
                            <el-form-item label="自动生成的账单模板类型:" class="postInfo-container-item">
                              <el-select v-if="$route.params.type!='view'" :clearable="true" default-first-option filterable v-model="data.billTypeId" placeholder>
                                <el-option v-for="(item,key) in bllTemplateList" :key="key" :label="item.name" :value="item.id"></el-option>
                              </el-select>
                          ...
                          <el-col :span="24">
                            <el-form-item label="账单文档:" class="postInfo-container-item">
                              ...
                              <p v-for="item in materialList" v-else>
                                ...
                                <el-button type="text" class="el-icon-view" @click="preView(item)"></el-button>
                                <el-button type="text" class="el-icon-download" @click="down(`${item.address}`)"></el-button>
                              </p>
                            </el-form-item>
                          </el-col>
                        </el-row>
                        <el-row class="fullRow">
                          <el-col :span="24">
                            <el-form-item label="备注:" class="postInfo-container-item">
                                <el-input v-if="$route.params.type!='view'"  size="small" type="textarea" :rows="3" v-model="data.remarks" placeholder="请输入"></el-input>
...
<!--                            <el-button v-if="fullScreenBox"  style="margin-left:10px" type="primary" @click="fullScreenBox=''" size="mini">还原</el-button>-->
<!--                            <el-button v-if="!fullScreenBox" style="margin-left:10px" type="primary" @click="fullScreen('billDetailsfwf')" size="mini">全屏</el-button>-->
                            <el-button v-if="$route.params.type!='view'"  style="margin-left:10px" type="primary" @click="addfwf('billDetailsfwf')" size="mini">添加</el-button>
                            <el-button v-if="$route.params.type!='view'"  style="margin-left:10px" type="primary" @click="delList('billDetailsfwf')" size="mini">删除</el-button>
                          </el-col>
                        </el-row>
...
<!--                              <el-date-picker v-if="$route.params.type!='view'"  size="small" v-model="scope.row.startdate " type="datetime" placeholder="选择日期时间" align="right" format="yyyy-MM-dd" value-format="yyyy-MM-dd" :picker-options="pickerOptions1"></el-date-picker>-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-date-picker v-if="$route.params.type!='view'"  size="small" format="yyyy-MM-dd" value-format="yyyy-MM-dd" v-model="scope.row.enddate " type="datetime" placeholder="选择日期时间" align="right" :picker-options="pickerOptions1"></el-date-picker>-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-popover v-if="$route.params.type!='view'"  v-model="scope.row.fwfPopShow" width class="inputshow" placement="bottom" trigger="click" >-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-select v-if="$route.params.type!='view'"  size="small" default-first-option ref :clearable="true" @change="((val)=>{ygchangeselect(val, scope.row,scope.$index)})" v-model="scope.row.userid" no-match-text="暂无数据" loading-text="正在查询" filterable remote reserve-keyword placeholder="请输入关键词" :remote-method="remotepaternalUnitYg">-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-select v-if="$route.params.type!='view'" @change="(e)=>changeRole(e,scope.$index)"  size="small" default-first-option  :clearable="true" v-model="scope.row.roleid" no-match-text="暂无数据" loading-text="正在查询" filterable remote reserve-keyword placeholder="请输入关键词">-->
<!--                                <el-option v-for="(item,key) in allRoles" :key="key" :label="item.name" :value="item.roleId"></el-option>-->
<!--                              </el-select>-->
<!--&lt;!&ndash;                              <el-input v-if="$route.params.type!='view'"  size="small"  type="text"  v-model="scope.row.roleid"></el-input>&ndash;&gt;-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input v-if="$route.params.type!='view'"  size="small"  type="number" min="0" @change="((val)=>{numberchange(val, scope.row,scope.$index)})" v-model="scope.row.number"></el-input>-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input   size="small" v-if="scope.row['localpriceState']" v-show="$route.params.type!='view'"  @blur="$set(scope.row,'localpriceState',false)" type="number" @change="((val)=>{djchange(val, scope.row,scope.$index)})" v-model="scope.row.localprice"></el-input>-->
<!--                              <el-input v-if="!scope.row['localpriceState']"  v-show="$route.params.type!='view'"  size="small" @focus="$set(scope.row,'localpriceState',true)" :value="floatFomat(scope.row.localprice)"></el-input>-->
<!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.localprice)}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
<!--                          <el-table-column-->
<!--                            prop="localsum"-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input  size="small"  v-if="scope.row['localsumState']" v-show="$route.params.type!='view'"  @blur="$set(scope.row,'localsumState',false)" type="number" @change="((val)=>{bbchange(val, scope.row,scope.$index)})" v-model="scope.row.localsum"></el-input>-->
<!--                              <el-input   size="small" v-if="!scope.row['localsumState']" v-show="$route.params.type!='view'" @focus="$set(scope.row,'localsumState',true)" :value="floatFomat(scope.row.localsum)"></el-input>-->
<!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.localsum)}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
<!--                          <el-table-column-->
<!--                            prop="discount"-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input v-if="$route.params.type!='view'"  size="small" @change="((val)=>{zkchange(val, scope.row,scope.$index)})" type="number" v-model="scope.row.discount" min="1" max="100"></el-input>-->
<!--                              <span v-if="$route.params.type=='view'">{{scope.row.discount}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
...
<!--                              <el-input v-show="$route.params.type!='view'" size="small"  v-if="scope.row['localSumDiscountState']" @blur="$set(scope.row,'localSumDiscountState',false)" @change="((val)=>{bbzkchange(val, scope.row,scope.$index)})" type="number" v-model="scope.row.localSumDiscount"></el-input>-->
<!--                              <el-input v-show="$route.params.type!='view'"  size="small" v-if="!scope.row['localSumDiscountState']" @focus="$set(scope.row,'localSumDiscountState',true)" :value="floatFomat(scope.row.localSumDiscount)"></el-input>-->
<!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.localSumDiscount)}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input v-show="$route.params.type!='view'"   size="small" v-if="scope.row['foreignpriceState']" @blur="$set(scope.row,'foreignpriceState',false)" @change="((val)=>{wbdjchange(val, scope.row,scope.$index)})" type="number" v-model="scope.row.foreignprice"></el-input>-->
<!--                              <el-input v-show="$route.params.type!='view'"  size="small" v-if="!scope.row['foreignpriceState']" @focus="$set(scope.row,'foreignpriceState',true)" :value="floatFomat(scope.row.foreignprice)"></el-input>-->
<!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.foreignprice)}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input v-show="$route.params.type!='view'"  size="small"   v-if="scope.row['foreignPriceDiscountState']" @blur="$set(scope.row,'foreignPriceDiscountState',false)" @change="((val)=>{wbdjzkhchange(val, scope.row,scope.$index)})" type="number" v-model="scope.row.foreignPriceDiscount"></el-input>-->
<!--                              <el-input v-show="$route.params.type!='view'"   size="small" v-if="!scope.row['foreignPriceDiscountState']" @focus="$set(scope.row,'foreignPriceDiscountState',true)" :value="floatFomat(scope.row.foreignPriceDiscount)"></el-input>-->
<!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.foreignPriceDiscount)}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input v-show="$route.params.type!='view'"  size="small"  v-if="scope.row['foreignsumState']" @blur="$set(scope.row,'foreignsumState',false)"  @change="((val)=>{wbjechange(val, scope.row,scope.$index)})" type="number" v-model="scope.row.foreignsum"></el-input>-->
<!--                              <el-input v-show="$route.params.type!='view'"   size="small" v-if="!scope.row['foreignsumState']" @focus="$set(scope.row,'foreignsumState',true)" :value="floatFomat(scope.row.foreignsum)"></el-input>-->
<!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.foreignsum)}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
...
<!--                            <template slot-scope="scope">-->
<!--                              <el-input v-show="$route.params.type!='view'"   size="small"   v-if="scope.row['foreignSumDiscountState']" @blur="$set(scope.row,'foreignSumDiscountState',false)" @change="((val)=>{wbjezkhchange(val, scope.row,scope.$index)})" type="number" v-model="scope.row.foreignSumDiscount"></el-input>-->
<!--                              <el-input v-show="$route.params.type!='view'"   size="small" v-if="!scope.row['foreignSumDiscountState']" @focus="$set(scope.row,'foreignSumDiscountState',true)" :value="floatFomat(scope.row.foreignSumDiscount)"></el-input>-->

<!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.foreignSumDiscount)}}</span>-->
<!--                            </template>-->
<!--                          </el-table-column>-->
<!--                          <el-table-column label="操作" align="left" width v-if="$route.params.type!='view'" width="40">-->
<!--                            <template slot-scope="scope">-->
...
<!--                        </div>-->
                        <div class="billallPice">
                          <span class="billtotalspan">数量合计：{{data.billDetailsfwf?data.billDetailsfwf.length:'0'}}条</span>
...
<!--                              <el-button v-if="fullScreenBox"  style="margin-left:10px" type="primary" @click="fullScreenBox=''" size="mini">还原</el-button>-->
<!--                              <el-button v-if="!fullScreenBox" style="margin-left:10px" type="primary" @click="fullScreen('billDetailgf')" size="mini">全屏</el-button>-->
                              <el-button v-if="$route.params.type!='view'"  style="margin-left:10px" type="primary" @click="addfwf('billDetailgf')" size="mini">添加</el-button>
                              <el-button v-if="$route.params.type!='view'"  style="margin-left:10px" type="primary" @click="delList('billDetailgf')" size="mini">删除</el-button>
                            </el-col>
                          </el-row>
                          ...
                          <div class="billallPice">
                            <span class="billtotalspan">数量合计：{{data.billDetailgf?data.billDetailgf.length:'0'}}条</span>
...
<!--                              <el-button v-if="fullScreenBox"  style="margin-left:10px" type="primary" @click="fullScreenBox=''" size="mini">还原</el-button>-->
<!--                              <el-button v-if="!fullScreenBox" style="margin-left:10px" type="primary" @click="fullScreen('billDetailzf')" size="mini">全屏</el-button>-->

                              <el-button v-if="$route.params.type!='view'"  style="margin-left:10px" type="primary" @click="addfwf('billDetailzf')" size="mini">添加</el-button>
                              <el-button v-if="$route.params.type!='view'"  style="margin-left:10px" type="primary" @click="delList('billDetailzf')" size="mini">删除</el-button>
                            </el-col>
                          </el-row>
  ...
  <!--                            <template slot-scope="scope">-->
  <!--                              <el-date-picker size="small" v-if="$route.params.type!=='view'" v-model="scope.row.startdate " type="datetime" placeholder="选择日期时间" align="right" format="yyyy-MM-dd" value-format="yyyy-MM-dd" :picker-options="pickerOptions1"></el-date-picker>-->
  <!--                              <span v-if="$route.params.type=='view'">{{scope.row.startdate}}</span>-->
  <!--                            </template>-->
  <!--                          </el-table-column>-->
  ...
  <!--                            <template slot-scope="scope">-->
  <!--                              <el-popover v-if="$route.params.type!='view'" v-model="scope.row.zfPopShow" class="inputshow" placement="bottom" trigger="click">-->
  ...
  <!--                              </el-popover>-->
  <!--                              <span v-if="$route.params.type=='view'">{{scope.row.description}}</span>-->
  <!--                            </template>-->
  <!--                          </el-table-column>-->
  ...
  <!--                            <template slot-scope="scope">-->
  <!--                              <el-input  size="small" v-if="scope.row['localsumState']" v-show="$route.params.type!='view'" @blur="$set(scope.row,'localsumState',false)"   type="number" @change="((val)=>{bbchangezf(val, scope.row,scope.$index)})" v-model="scope.row.localsum"></el-input>-->
  <!--                              <el-input size="small" v-if="!scope.row['localsumState']" v-show="$route.params.type!='view'" @focus="$set(scope.row,'localsumState',true)" :value="floatFomat(scope.row.localsum)"></el-input>-->
  <!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.localsum)}}</span>-->
  <!--                            </template>-->
  <!--                          </el-table-column>-->
  ...
  <!--                            <template slot-scope="scope">-->
  <!--                              <el-input size="small" v-if="scope.row['foreignsumState']" v-show="$route.params.type!='view'" @blur="$set(scope.row,'foreignsumState',false)"   @change="((val)=>{wbjechangezf(val, scope.row,scope.$index)})" type="number" v-model="scope.row.foreignsum"></el-input>-->
  <!--                              <el-input size="small" v-if="!scope.row['foreignsumState']" v-show="$route.params.type!='view'" @focus="$set(scope.row,'foreignsumState',true)" :value="floatFomat(scope.row.foreignsum)"></el-input>-->
  <!--                              <span v-if="$route.params.type=='view'">{{floatFomat(scope.row.foreignsum)}}</span>-->
  <!--                            </template>-->
  <!--                          </el-table-column>-->

  <!--                          <el-table-column label="操作" align="left" width v-if="$route.params.type!='view'">-->
  <!--                            <template slot-scope="scope">-->
  ...
  <!--                        </el-table>-->
                          ...
                          <div class="billallPice">
                            <span class="billtotalspan">数量合计：{{data.billDetailzf?data.billDetailzf.length:'0'}}条</span>
  ...
  <!--                          {{Number(totalPrincipal_gf)+Number(totalPrincipal_fw_zh+Number(totalPrincipal_zf))}}-->
                        ...
                        <div v-show="!createCommonBill">
                          <el-row style="border-left:none">
                            <el-col class="tilteSpan" :span="24">
                              ...
                              <el-button v-if="$route.params.type!='view' || ['5', '6'].includes($route.query.taskNo)"  style="margin-left:10px;float: right" type="primary" @click="data.billAllocations.push({wkgid:'',groupType:''})" size="mini">添加</el-button>
                            </el-col>
                          </el-row>
                          <el-table size="mini" :data="data.billAllocations" border fit empty-text="暂无数据" current-row-key style="width: 100%;">
                            <el-table-column label="工作组类型" align="left" width="210">
                              <template slot-scope="scope">
                                <el-select v-if="$route.params.type!='view' || ['5', '6'].includes($route.query.taskNo)"  v-model="scope.row.groupType" size="small"  no-match-text="暂无数据" filterable >
                                  <el-option v-for="(item,key) in ['客户组','承办组']" :key="key" :label="item" :value="item"></el-option>
                                </el-select>
                            ...
                            <el-table-column label="工作组" align="left">
                              <template slot-scope="scope">
                                <el-select v-if="$route.params.type!='view' || ['5', '6'].includes($route.query.taskNo)"  v-model="scope.row.wkgid" size="small"  no-match-text="暂无数据" filterable >
                                  <el-option v-for="(item,key) in selectData.gzzs" :key="key" :label="item.groupName" :value="item.wkgId"></el-option>
                                </el-select>
                            ...
                            <el-table-column label="分配比例" align="left" width>
                              <template slot-scope="scope">
                                <div v-if="$route.params.type!='view' || ['5', '6'].includes($route.query.taskNo)">
                            ...
                            <el-table-column label="分配金额" align="left" width>
                              <template slot-scope="scope">
                                <el-input size="small" v-if="scope.row['allomoneyState']" v-show="$route.params.type!='view' || ['5', '6'].includes($route.query.taskNo)" @blur="$set(scope.row,'allomoneyState',false)"  type="number"  v-model="scope.row.allomoney" @change="((val)=>{jechangeinput(val, scope.row,scope.$index)})" placeholder></el-input>
                                <el-input size="small" v-if="!scope.row['allomoneyState']" v-show="$route.params.type!='view' || ['5', '6'].includes($route.query.taskNo)" @focus="$set(scope.row,'allomoneyState',true)" :value="floatFomat(scope.row.allomoney)"></el-input>
                                <span v-if="$route.params.type=='view' && !['5', '6'].includes($route.query.taskNo)">{{floatFomat(scope.row.allomoney)}}</span>
                              </template>
                            </el-table-column>
                            ...
                            <el-table-column label="备注" align="left" width>
                              <template slot-scope="scope">
                                <el-input @dblclick.native="openEditDialog(scope.row,scope.$index)" v-if="type!='view'" type="textarea" :rows="1" size="small" v-model="scope.row.memo" placeholder="请输入"></el-input>
                                <span v-else>{{ scope.row.memo }}</span>
                              </template>
                            </el-table-column>
                            <el-table-column label="操作" align="left" width v-if="$route.params.type!='view'">
                              <template slot-scope="scope">
                                <el-button type="text" @click="data.billAllocations.splice(scope.$index,1)" size="mini">删除</el-button>
                              </template>
                            </el-table-column>
                          </el-table>
                          <el-row class="fullRow">
                ...
                <el-row style="margin-top:40px" v-if="$route.params.type!='view' || ['5', '6'].includes($route.query.taskNo)">
                  <el-col :span="24" style="text-align:center">
                    ...
                    <template v-else>
                      ...
                      <el-button :disabled="flag" size="medium" v-if="$route.params.type=='creat'" @click="sunmitBill(1)" type="primary">提交</el-button>
                      <el-button :disabled="flag" size="medium" v-if="($route.query.taskNo=='1')&&$route.query.taskNo" @click="shfunc('1')" type="primary">提交</el-button>
                      <el-button :disabled="flag" size="medium" v-if="($route.query.taskNo=='6')&&$route.query.taskNo" @click="shfunc('1')" type="primary">确认比例</el-button>
                      <el-button :disabled="flag" size="medium" v-if="$route.query.taskNo!='1'&&$route.query.taskNo!='6'&&$route.query.taskNo" @click="shfunc('1')" type="primary">通过</el-button>
                      <el-popover
                        placement="top"
                        width="250"
                        title="退回原因"
                        v-if="$route.query.taskNo!='1'&&$route.query.taskNo!='6'&&$route.query.taskNo"
                      >
                        <div style="text-align: right">
                          ...
                          <el-button type="primary" @click="shfunc('0',failReasion)" class="exactButton" size="mini">确认退回</el-button>
                        </div>
                        <el-button slot="reference" :disabled="flag" size="medium" type="primary">退回</el-button>
                      </el-popover>
                    </template>

                    <el-button :disabled="flag" style="margin-left:50px" type @click="cancel" size="medium">取消</el-button>
                  </el-col>
                </el-row>
                <el-row style="margin-top:40px" v-else>
                  <el-col :span="24" style="text-align:center">
                    <el-button  type @click="cancel" size="medium">关闭</el-button>
                  </el-col>
                </el-row>
                <el-row>
                  <el-col class="form-con-item" :span="24">
                      ...
                      <el-row>
                        <el-col :span="12">
                          <el-form-item label="我方账户:" :prop="createCommonBill ? '' : 'payAccountId'" class="postInfo-container-item">
                            <el-select v-if="$route.params.type!='view'" filterable clearable default-first-option @change="yhSelectchange" v-model="data.payAccountId" placeholder>
                              <el-option v-for="(item,key) in selectData.yhData.filter(item => $store.getters.permissions.includes(364) && item.payAccountId == 49 || item.payAccountId != 49)" :key="key" :label="item.payAccountName" :value="item.payAccountId"></el-option>
                            </el-select>
                        ...
                        <el-col :span="12">
                          <el-form-item label="是否使用英文账户:" class="postInfo-container-item">
                            <el-radio-group  v-if="$route.params.type!='view'" @change="ywradiochange" v-model="data.useEnAccount">
                              <el-radio :label="1">是</el-radio>
                              <el-radio :label="0">否</el-radio>
                            </el-radio-group>
                            <div v-else>
       ...
       <el-tab-pane label="核销信息" name="second">
         <transition name="show" mode="out-in">
           <WriteOff @billTypeEvent="billTypeEvent" ref="writeOff" v-if="data.billId" :type="$route.params.type" :billId="data.billId" @changeBill="changeBill" :caseIds="data.caseArray.map(item=>item.caseId)"></WriteOff>
         </transition>
       </el-tab-pane>
...
<!--                <el-input type="textarea"  autosize v-if="$route.params.type!='view'" size="small" :title="scope.row.specialInfo" v-model="scope.row.specialInfo" placeholder></el-input>-->
...
<!--          </el-table-column>-->
<!--        </el-table>-->
        <SpecialInstructions :type="$route.params.type" :data="data"></SpecialInstructions>
      </div>
    </div>
    <el-dialog :title="`${data.custFullNameString}——联系人/抬头编辑`" :visible.sync="dialogVisibleTt" width="70%">
      <CustDetail v-if="dialogVisibleTt" :is-edit="$route.params.type!='view'" :billCustId="data.custId" :isBillEdit="true" />
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.createCommonBill) {
      this.createCommonBill = true
    }
    if (this.$route.query.billIdList) {
      this.billIdList = this.$route.query.billIdList
    }
  ...
  data() {
    ...
    return {
      ...
      timeDefaultShow:'',
      progressData:[],
      progressColumn:[{prop: 'progressName',label:'任务名称'},{prop: 'status',label:'任务状态'},{prop: 'tasker',label:'任务人'},{prop: 'createTime',label:'创建时间'},{prop:'result',label:'任务执行结果'},{prop:'returnReason',label:'退回原因'}],
      materialList:[],
      creatematerial,
      ...
      preferenceList:[
      ...
      },
        {
          ...
          resizable: true,
          sortable: true,
          editable: this.$route.params.type !== 'view',
  ...
  methods: {
      ...
      checkBillDescWord({sentence: this.data.caseDesc || ''}).then(res => {
        res.data && res.data.forEach(item => {
          ...
          html = html.split(' ').map(i => {
            ...
            if (i.replace(/[^a-zA-Z]/g, '') == item) return i.replace(reg, `<span class="error-text" style="text-decoration: underline wavy red;font-size: 13px">${item}</span>`)
            return i
          }).join(' ')
        })
    ...
    billTypeEvent(v){
      v.billStyle&&(this.data.billStyle = v.billStyle)
      // this.data.billStyle = v.billStyle
      // this.data.billStyleName = v.billStyleName
    },
    fullScreen(type){
    ...
    changeRowFwf(event){
      ...
      if(event.colDef.field=='discount'){
        this.zkchange(event.value,event.data,index)
      }
    ...
    cellRenderer(params){
      ...
      if(params.colDef.field=='roleid'){
        return `<span title="${params.data.roleidString}">${params.data.roleidString || ''}</span>`
      }
    ...
    // },
    // previewDoc(file){
    //   if(file.raw && file.response && this.$store.getters.permissions.includes(163)){
    //       // 修改账单文档权限限制
    //         this.doEditInOffice(file.response.data[0].address)
    //       // this.$commonUtils.downLoadAll({url:'ipdoc'+file.response.data.address})
    //   } else {
    ...
    queryBillTaskRecord() {
      queryBillTaskRecord({ id: this.billId ,typeId:8}).then(res => {
    ...
    checkWkgIsGj(e, flag){
      ...
      if (this.createCommonBill && !flag) {
          checkWkg({wkgId: e, billIdList: this.billIdList}).then(res => {
            ...
            this.$confirm(`${err.message}, 是否清除账单分配比例？`, "提示", {
              ...
              deleteBillAllocation({billIdList: this.billIdList}).then(res => {
    ...
    cancel(flag){
      ...
      if(this.type==2){
        ...
        if (!this.isFinance){
          this.$router.push('/workbench/finance_bill_manager/finance_bill')
        }else{
          this.$emit('closeBillDialog',false) // 关闭父级弹框
        }
      }
    },
        ...
        if (valid) {
          ...
          updateBillUrl(reqData).then(res => {
            batchdoBillTaskUrl({
              ...
              saveLog({content:JSON.stringify({billId:this.data.billId,message:'审核报错'+e.message,logType:1})}).then(res=>{
    ...
    getBilldetail() {
      ...
      if (!this.isFinance) {
        ...
        if (this.$route.query.taskNo) {
          if (
            this.$route.query.taskNo != "1" &&
            this.$route.query.taskNo != "6"
          ) {
      ...
      queryBillUrl({ billId: id }).then(res => {
        ...
        // if(this.data.taskType==4&&this.billTypeList.length ==0){
        //   this.queryBillStyle()
        // }
  ...
  async  getallSelectdata() {
    ...
    fwfinputchange(val) {
      if (val != "") {
        queryChargeItemBynameCnUrl({ nameCn: val,billId:this.billId }).then(res => {
    ...
    async sunmitBill(num) {
      ...
      await this.handleNullFee()
      // let bl = 0
      //   this.data.billAllocations.forEach(item=>{
      //     if(item.alloratio){
      //       bl += +item.alloratio
      //     }
      //   })
      //   if(bl < 99.9){
      //     this.$message.error('分配比例总和应该为100')
      //     return
      //   }
      if (this.data.billAllocations) {
         let number_f = 0;
            ...
            if (num == 1) {
              ...
              createBillUrl(reqData).then(res => {
               ...
               // this.$router.push({ name: "bill" ,params: { siveId: this.data.billId }});
                ...
                saveLog({content:JSON.stringify({billId:this.data.billId,message:'审核报错'+e.message,logType:2})}).then(res=>{
              ...
              delete reqData.billNo;
              this.flag=true
              if (this.billIdList && this.billIdList.length) {
                reqData.billIdList = this.billIdList
              }
              updateBillUrl(reqData).then(res => {
                this.flag=false
                this.$message.success(res.message);
