<template>
  <div class="staff">
    <MyTabs ref="myTabs" class="MyTabs" :page-title="'样品管理'" :identification="identification" :defaultColumnDefs="columnDefs"  @editableTab="editableTab" :class="myTabsActivity?'activity':''"></MyTabs>
    ...
    <div class="archiveButton">
      ...
      <template v-if="this.$store.getters.permissions.includes(371)">
        <el-button size="small" style="margin-left: 15px" @click="handleCreateView()" type="primary">新建样品</el-button>
        ...
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item @click.native="sampleCount(1)">今日新建样品</el-dropdown-item>
            <el-dropdown-item @click.native="sampleCount(2)">今日借出样品</el-dropdown-item>
            <el-dropdown-item @click.native="sampleCount(3)">今日归还样品</el-dropdown-item>
            <el-dropdown-item @click.native="sampleCount(7)">今日已销毁</el-dropdown-item>
            <el-dropdown-item @click.native="sampleCount(4)">未销毁</el-dropdown-item>
            <el-dropdown-item @click.native="sampleCount(5)">已销毁</el-dropdown-item>
            <el-dropdown-item @click.native="sampleCount(6)">异地保存</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        <el-dropdown>
          <el-button size="small" type="primary">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item @click.native="customExport(2)">寄出</el-dropdown-item>
            <el-dropdown-item @click.native="customExport(3)">销毁</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
...
<!--            </div>-->
      ...
      <div class="block">
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="寄出数量">
            <template slot-scope="scope">
              <span>{{ scope.row.mailCount ? scope.row.mailCount : 0 }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="returnCount"
    ...
    <el-dialog :visible.sync="detailView" title="样品信息" width="50%" :close-on-click-modal="false" :close-on-press-escape="false">
      <sampleDetail v-if="detailView" :check-type="checkType" :sample-id="sampleId" @closeDialog="closeDetailDialog"></sampleDetail>
    </el-dialog>

    <el-dialog :visible.sync="settingView" :title="settingName" width="60%" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
...
import { querySampleList, delSample, copySample, sampleWorkloadStatistics, exportSampleLendOut, exportSampleSendOut, exportSampleDestroy } from "@/api/material";
...
export default {
  ...
  methods: {
    ...
    // 按工作量统计
    queryWorkloadData() {
      sampleWorkloadStatistics(this.workloadData).then(res => {
    ...
    customExport(type) {
      if (this.getSelectedRows().length > 0){
        ...
        if(type == 1) {
          ...
          exportSampleSendOut(queryData).then(res => {
            this.downLoad(res.data)
          })
        }else if(type == 3) {
          exportSampleDestroy(queryData).then(res => {
            this.downLoad(res.data)
          })
        }
      }else{
    ...
    getContextMenuItems(params){
      return [{name:'复制',
        ...
        {name:'复制样品',
          action:()=>{
            this.copySample(params.node.data)
          },permissions:371
        },
        {name:'修改',
          action:()=>{
        ...
        {name:'删除',
          action:()=>{
            this.deleteSamId(params.node.data.samId)
    ...
    temporarStorageAll(){
      ...
      this.$refs.myTabs.tabClick({name: '暂存样品'}, true)
    },
    editableTab(data) {
      if (data && data != 0) {
    ...
    cellRenderer(params){
      ...
      if(['quantity','sendNum','mailNum','destroyedNum','receiveTotal'].includes(params.colDef.field)){
