...
const dict = pinyin_dict_notone
const allPinyin=Object.keys(pinyin_dict_notone)
...
function match(input, keys) {
  ...
  // console.log(utils.getPinyin(input),getCombination(utils.getPinyin(input)));
  // let py = utils.getPinyin(input).map(item=>item[0])
  let fullString = storage[keys] || getFullKey(keys)
  return getCombination(utils.getPinyin(input)).some(item=>getIndex(item, fullString, keys))// getIndex(py, fullString, keys)
}
